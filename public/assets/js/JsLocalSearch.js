...
(function ($, window, document) {
  var JsLocalSearch = {
    init: function (options, el) {
      ...
      var searchinput =
        options.searchinput == undefined
          ? "#gsearchsimple"
          : options.searchinput;
      options.container =
        options.container == undefined ? "contsearch" : options.container;
      var containersearch =
        options.containersearch == undefined
          ? "gsearch"
          : options.containersearch;
      var mincaracteres =
        options.mincaracteres == undefined ? 3 : options.mincaracteres;
      var action = options.action == undefined ? "Show" : options.action;
      var info = options.info == undefined ? false : options.info;
      options.mark_text =
        options.mark_text == undefined ? false : options.mark_text;
      options.html_search =
        options.html_search == undefined ? true : options.html_search;
          ...
          $("." + options.container)
            ...
            .each(function () {
              if (options.mark_text != undefined) {
                var re = new RegExp(
                  '<span class="' + options.mark_text + '">(.*?)</span>',
                  "gi"
                );
                var str = $(this).html();
    ...
    busca: function (options, actionObject, el) {
        ...
        if (
          ...
          if (options.mark_text != undefined) {
            var newstr = str.replace(
              re,
              "<span class='" + options.mark_text + "'>$1</span>"
            );
          }
          $(el).html(newstr);
        }
      ...
      if (
        $(el).attr("data-description") != undefined &&
        options.description_search
      ) {
        if (
          $(el)
  ...
  var ActionMark = {
    ...
    ko: function (t) {
      if (
        ...
        $(t)
          .closest("." + this.container)
          .removeClass(this.mark);
        $(t)
          .closest("." + this.container)
          .addClass(this.unmark);
      }
    },
    nope: function () {
      $("." + this.container).removeClass(this.mark);
