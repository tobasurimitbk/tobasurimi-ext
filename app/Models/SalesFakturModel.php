...
class SalesFakturModel extends Model
{
    protected $SalesFakturDetailModel;

    protected $table      = 'sales_faktur';
    ...
    public function getAllSalesOrderLokal($condition, $addCondition, $limit = 10, $offset = 0)
    {
        //disini
        $availableSort = [
            'no_sales_order'          => 'sales_faktur.no_sales_order',
            'order_date'          => 'sales_faktur.order_date',
            'destination'            => 'sales_faktur.destination',
            'qty_barang'             => 'sales_faktur.qty_barang',
            'total_harga'             => 'sales_faktur.total_harga',
            'salesName'             => 'employees.name',
            'company'             => 'companies.company',
            'keterangan'      => 'sales_faktur.keterangan',
            'createdAt'         => 'sales_faktur.createdAt',
            'updatedAt'         => 'sales_faktur.updatedAt',
        ];
        ...
        if ($addCondition['dateStart']) {
            $salesOrderLokal->where('sales_faktur.order_date >=',  $addCondition['dateStart']);
        }
        if ($addCondition['dateEnd']) {
            $salesOrderLokal->where('sales_faktur.order_date <=', $addCondition['dateEnd']);
        }

        if ($addCondition['filter_customer']) {
            $salesOrderLokal->where('sales_faktur.id_customer', $addCondition['filter_customer']);
        }

        if ($addCondition['filter_invoice'] == "belum") {
            $salesOrderLokal->where('sales_faktur.sales_order_invoice_id', NULL);
        }
        ...
        if ($addCondition['filter_surat_jalan'] == "belum") {
            $salesOrderLokal->where('sales_faktur.surat_jalan_so_id', NULL);
        }
    ...
    public function getSalesOrderLokalById($id)
    {
        ...
        $dataSalesOrder = $this->asObject()
            ->join('users', 'users.id = sales_faktur.id_user', 'left')
            ->join('customers', 'customers.id = sales_faktur.id_customer ', 'left')
            ->join('metadata as termin', 'termin.id = sales_faktur.payment_terms', 'left')
            ->join('metadata as tipe_pelanggan', 'tipe_pelanggan.id = customers.tipe_pelanggan', 'left')
            ->join('employees', 'employees.id = sales_faktur.sales_id ', 'left')
            ->select($selectQry)
            ->find($id);

        $selectQueryDetail = "sales_faktur_detail.*,
                              ...
                              sales_faktur_detail.harga_barang AS harga_barang,
                              barang_master_sales.satuan_id,
                              satuans.kode_satuan AS satuan";
        $detail = $this->SalesFakturDetailModel
            ...
            ->join('warehouses', 'warehouses.id = sales_faktur_detail.id_warehouse', 'left')
