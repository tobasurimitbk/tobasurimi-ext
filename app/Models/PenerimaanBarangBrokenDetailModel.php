...
class PenerimaanBarangBrokenDetailModel extends Model
{
    protected $DBGroup          = 'default';
    protected $table            = 'penerimaan_barang_broken_detail';
    ...
    {
        $selectQry = "penerimaan_barang_broken_detail.*, warehouses.warehouse_name, metadata.value as aju_type, 
        suppliers.name as supplier_name, penerimaan_barang_broken.validation_date,
        barang_master.kode_barang, 
        barang_master.barang_name nama_barang, 
        penerimaan_barang_broken.tipe_bahan,
        satuans.nama_satuan,
        satuans.kode_satuan,
        penerimaan_barang_broken.no_penerimaan_barang, penerimaan_barang_broken.multiple_po_no";
        $penerimaanBarangDataQry = $this->asObject()
            ...
            ->join('metadata', 'metadata.id = penerimaan_barang_broken.aju_document_type', 'left')
            ...
            ->join('satuans', 'penerimaan_barang_broken_detail.unit = satuans.id', 'left')
            ->where($condition);
        ...
        if ($addCondition['search']) {
            $penerimaanBarangDataQry->like('penerimaan_barang_broken.no_penerimaan', $addCondition['search']);
        }

        if ($addCondition['status']) {
            $penerimaanBarangDataQry->where('penerimaan_barang_broken.status_post', $addCondition['status']);
        }

        if ($addCondition['startdate']) {
            $penerimaanBarangDataQry->where('penerimaan_barang_broken.validation_date >=', $addCondition['startdate']);
        }

        if ($addCondition['lastdate']) {
            $penerimaanBarangDataQry->where('penerimaan_barang_broken.validation_date <=', $addCondition['lastdate']);
        }
    ...
    public function getPenerimaanBarangDetailByPenerimaanBarangId($id, $tipe_bahan, $status_penerimaan)
    {
        $arrCondition = [
            ...
            'penerimaan_barang_broken_detail.penerimaan_barang_id' => $id
        ];
        ...
        if ($status_penerimaan === "LOKAL") {
            if ($tipe_bahan === "BAKU") {
                $builder->select(
                    "penerimaan_barang_broken_detail.*, satuans.id as id_satuan, 
                    penerimaan_barang_broken_detail.id AS penerimaan_barang_broken_detail_id,
                satuans.kode_satuan, 
                barang_master.kode_barang, 
                barang_master.barang_name as nama_barang,
                penerimaan_barang_broken_detail.ppn as id_ppn,
                penerimaan_barang_broken_detail.pph as id_pph,
                ...
                SUM(penerimaan_barang_broken_detail.jml_masuk) as jml_masuk,
                GROUP_CONCAT(IF(penerimaan_barang_broken_detail.keterangan IS NOT NULL AND penerimaan_barang_broken_detail.keterangan != '', penerimaan_barang_broken_detail.keterangan, NULL)) as keterangan_lpb"
                )
                    ->where($arrCondition)
                    ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id', 'LEFT')
                    ->join('taxes as ppn', 'ppn.id = penerimaan_barang_broken_detail.ppn', 'LEFT')
                    ->join('taxes as pph', 'pph.id = penerimaan_barang_broken_detail.pph', 'LEFT')
                    ->join('satuans', 'satuans.id = penerimaan_barang_broken_detail.unit', 'LEFT')
                    ->join('rm_purchase_order_details', 'rm_purchase_order_details.id = penerimaan_barang_broken_detail.purchase_order_details_id', 'LEFT')
                    ...
                    ->groupBy('penerimaan_barang_broken_detail.penerimaan_barang_id');
                $query = $builder->get();
            }
            if ($tipe_bahan === "PENOLONG") {
                ...
                purchase_requests.spp_no'
                    ...
                    ->where($arrCondition)
                    ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id')
                    ->join('taxes as ppn', 'ppn.id = penerimaan_barang_broken_detail.ppn', 'LEFT')
                    ->join('taxes as pph', 'pph.id = penerimaan_barang_broken_detail.pph', 'LEFT')
                    ->join('satuans', 'satuans.id = penerimaan_barang_broken_detail.unit', 'LEFT')
    ...
    public function getPenerimaanBarangPenolongDetail($id)
    {
        return $this->asArray()->select('
                    ...
                    penerimaan_barang_broken_detail.sub_total AS total_penerimaan_detail,
                    barang_master.barang_name as nama_barang, 
                    ...
                    purchase_requests.spp_no,
                    sum(penerimaan_barang_broken_detail.sub_total) as total_penerimaan_detail
                ')
            ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id', 'left')
    ...
    {
        return $this->asArray()->select('penerimaan_barang_broken_detail.*,barang_master.barang_name as nama_barang, satuans.kode_satuan, rm_purchase_orders.po_no, rm_purchase_order_details.note as keterangan, supplier_harga.spesifikasi, rm_purchase_order_details.qty as qty_barang_po, rm_purchase_order_details.general_price as total_barang_po, barang_master_spesifikasi.spesifikasi, SUM(rm_purchase_order_details.general_price) as sum_total_barang_po, SUM(rm_purchase_order_details.qty) as sum_qty_barang_po, sum(penerimaan_barang_broken_detail.sub_total) as total_penerimaan_detail')
            ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id', 'left')
            ->join('barang_master_spesifikasi', 'barang_master_spesifikasi.id = penerimaan_barang_broken_detail.spesifikasi_id', 'left')
            ->join('satuans', 'satuans.id = penerimaan_barang_broken_detail.unit', 'left')
    ...
    {
        return $this->asArray()->select('penerimaan_barang_broken_detail.*,barang_master.barang_name as nama_barang, satuans.kode_satuan, rm_import_pos.po_no, rm_import_po_details.note as keterangan, rm_import_po_details.total as total_po, rm_import_pos.currency as currency, barang_master_spesifikasi.spesifikasi, metadata.value as currencyValue, SUM(rm_import_po_details.total) as sum_total_po, sum(penerimaan_barang_broken_detail.sub_total) as total_penerimaan_detail')
            ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id', 'left')
            ->join('barang_master_spesifikasi', 'barang_master_spesifikasi.id = penerimaan_barang_broken_detail.spesifikasi_id', 'left')
            ->join('satuans', 'satuans.id = penerimaan_barang_broken_detail.unit', 'left')
    ...
    {
        return $this->asArray()->select('SUM(penerimaan_barang_broken_detail.sub_total) AS harga')
    ...
    public function historiHargaPOBahanPenolongByLpb($condition, $addCondition, $limit = 10, $offset = 0)
    {
        $availableSort = [
            ...
            'penerimaan_barang_broken.tanggal' => 'penerimaan_barang_broken.tanggal',
            'suppliers.name'  => 'suppliers.name',
            'barang_master_spesifikasi.spesifikasi' => 'barang_master_spesifikasi.spesifikasi',
            'penerimaan_barang_broken_detail.sub_total'    => 'penerimaan_barang_broken_detail.sub_total',
            'penerimaan_barang_broken_detail.qty'    => 'penerimaan_barang_broken_detail.qty',
            'penerimaan_barang_broken_detail.harga'    => 'penerimaan_barang_broken_detail.harga',
            'penerimaan_barang_broken_detail.unit' => 'penerimaan_barang_broken_detail.unit',
            'penerimaan_barang_broken.divisi_id' => 'penerimaan_barang_broken.divisi_id',
            'penerimaan_barang_broken.no_penerimaan_barang' => 'penerimaan_barang_broken.no_penerimaan_barang',
        ...
        $selectQry = "
            ...
            penerimaan_barang_broken_detail.qty,
            penerimaan_barang_broken_detail.sub_total, 
            divisis.divisi,
            satuans.kode_satuan,
            purchase_requests.spp_no,
        ...
        $dataLPB = $this->asArray()
            ...
            ->join('am_purchase_orders', 'am_purchase_orders.id  = penerimaan_barang_broken_detail.purchase_order_id')
            ->join('am_purchase_order_details', 'am_purchase_order_details.id  = penerimaan_barang_broken_detail.purchase_order_details_id')
            ->join('purchase_requests', 'purchase_requests.id = am_purchase_orders.purchase_request_id', 'left')
            ->join('barang_master', 'barang_master.id = penerimaan_barang_broken_detail.barang_id', 'left')
            ->join('barang_master_spesifikasi', 'barang_master_spesifikasi.id = penerimaan_barang_broken_detail.spesifikasi_id', 'left')
            ...
            ->join('satuans', 'satuans.id = penerimaan_barang_broken_detail.unit', 'left')
            ->groupBy('penerimaan_barang_broken.id, nama_barang, po_no, tanggal, note, nama_supplier, divisi, kode_satuan, spp_no, no_penerimaan_barang')
        ...
        if ($addCondition['start_date'] && $addCondition['end_date']) {
            ...
            $dataLPB->where('penerimaan_barang_broken.tanggal >=', $startDate)
                ->where('penerimaan_barang_broken.tanggal <=', $endDate);
        }
        ...
        if ($addCondition['search']) {
            ...
            $dataLPB->like('purchase_requests.spp_no', $addCondition['search'])
                ...
                ->orLike('penerimaan_barang_broken_detail.qty', $addCondition['search'])
                ->orLike('penerimaan_barang_broken.no_penerimaan_barang', $addCondition['search'])
                ->orLike('satuans.kode_satuan', $addCondition['search'])
                ->orLike('penerimaan_barang_broken_detail.harga', $addCondition['search']);
            $dataLPB->groupEnd();
        }
    ...
    public function historiHargaPOBahanPenolongByLpbFirst(
        ...
        $condition = [
            ...
            "penerimaan_barang_broken.tipe_bahan" => $tipeBahan,
            "penerimaan_barang_broken.company_id"  => $companyId,
            "penerimaan_barang_broken_detail.spesifikasi_id" => $spesifikasiId,
            "penerimaan_barang_broken.deletedAt" => NULL,
            "penerimaan_barang_broken_detail.deletedAt" => NULL,
            "penerimaan_barang_broken.status_penerimaan" => $statusPenerimaan
        ];
        ...
        $selectQry = "
            ...
            am_purchase_orders.po_no,
            penerimaan_barang_broken.tanggal,
            penerimaan_barang_broken.supplier_id,
            am_purchase_order_details.note,
            suppliers.name as nama_supplier,
            penerimaan_barang_broken_detail.harga, 
            penerimaan_barang_broken_detail.qty,
    ...
    public function getHistoriLpbBahanPenolong($statusPenerimaaan, $tipeBahan, $poDetailId)
    {
        $penerimaanBarang = $this->asArray()
            ...
            ->join('penerimaan_barang_broken', 'penerimaan_barang_broken.id = penerimaan_barang_broken_detail.penerimaan_barang_id', 'left')
            ->where('penerimaan_barang_broken.status_penerimaan', $statusPenerimaaan)
            ->where('penerimaan_barang_broken.tipe_bahan', $tipeBahan)
            ->where('penerimaan_barang_broken_detail.purchase_order_details_id', $poDetailId)
            ->orderBy('penerimaan_barang_broken.tanggal', "desc")
            ->first();
