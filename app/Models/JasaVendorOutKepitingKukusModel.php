...
class JasaVendorOutKepitingKukusModel extends Model
{
    protected $DBGroup          = 'default';
    protected $table            = 'jasa_vendor_out_kepiting_kukus';
    ...
    public function getList($condition, $conditionArr, $addCondition, $limit = 10, $offset = 0)
    {
        $availableSort = [
            ...
            'jasa_vendor_out_kepiting_kukus.createdAt' => 'jasa_vendor_out_kepiting_kukus.createdAt',
            'jasa_vendor_out_kepiting_kukus.divisi_id' => 'jasa_vendor_out_kepiting_kukus.divisi_id',
            'jasa_vendor_out_kepiting_kukus.warehouse_id' => 'jasa_vendor_out_kepiting_kukus.warehouse_id',
            'vendor_id' => 'vendor_id',
        ...
        $selectQry = "jasa_vendor_out_kepiting_kukus.*,
        divisis.divisi,
        ...
        $dataQry = $this->asObject()
            ...
            ->join('vendors', 'vendors.id = jasa_vendor_out_kepiting_kukus.vendor_id', 'left')
            ->where($condition)
            ->whereIn('jasa_vendor_out_kepiting_kukus.divisi_id', $conditionArr)
        ...
        if ($addCondition['divisi_id']) {
            $dataQry->where('jasa_vendor_out_kepiting_kukus.divisi_id', $addCondition['divisi_id']);
        }

        if ($addCondition['warehouse_id']) {
            $dataQry->like('jasa_vendor_out_kepiting_kukus.warehouse_id', $addCondition['warehouse_id']);
        }
        ...
        foreach ($jasaVendorOutData as $j) {
            ...
            if ($stockListOutDetail) {
                ...
                $jasaVendorInDetail = $jasaVendorInDetailModel
                    ->where('jasa_vendor_out_kepiting_kukus_id', $jasaVendorInID)
                ...
                $jasaVendorIn = $jasaVendorInModel
                    ->select('jasa_vendor_out_kepiting_kukus.*,vendors.name as nama_vendor')
                    ->join('vendors', 'vendors.id = jasa_vendor_out_kepiting_kukus.vendor_id', 'left')
                    ->where('no_penerimaan_surat_jalan', $noJasaVendorIn)
                    ->where('jasa_vendor_out_kepiting_kukus.company_id',  session()->get("login")->this_company_id)
                    ->first();
    ...
    public function get_no($bln, $thn, $divisi)
    {
        ...
        $builder = $this->db->table('jasa_vendor_out_kepiting_kukus');
