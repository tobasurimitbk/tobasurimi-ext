...
class ProductionResultModel extends Model
{
    ...
    public function getDataProductionResultWithDetail($where)
    {
        ...
        $dataQry = $this->asArray()
            ...
            ->join('account_barang', 'account_barang.barang_master_id = production_result_details.barang1_id AND account_barang.barang_master_spesifikasi_id = production_result_details.barang2_id', 'left')
    ...
    public function getDataProductionResultBahanBakuWithDetail($where)
    {
        ...
        $dataQry = $this->asArray()
            ->select($selectQryJadi)
            ->join('production_result_details', 'production_result_details.production_result_id = production_results.id', 'left')
            ->join('barang_master', 'barang_master.id = production_result_details.barang1_id')
            ->join('barang_master_spesifikasi', 'barang_master_spesifikasi.id = production_result_details.barang2_id')
            ->join('satuans', 'satuans.id = barang_master_spesifikasi.satuan_1', 'left')
            ->join('work_orders', 'work_orders.id = production_results.work_order_id', 'left')
            ->join('account_barang', 'account_barang.barang_master_id = production_result_details.barang1_id AND account_barang.barang_master_spesifikasi_id = production_result_details.barang2_id')
