...
class InvPackingBcModel extends Model
{
    protected $DBGroup          = 'default';
    protected $table            = 'inv_packing_bc';
    ...
    public function getList($condition, $addCondition, $limit = 10, $offset = 0)
    {
        ...
        $sort = $availableSort[$addCondition['sort'] ?? 'createdAt'] ?? 'inv_packing_bc.id';
        $sortType = $availableSortType[$addCondition['sortType'] ?? 'desc'] ?? 'DESC';

        $selectQry = "inv_packing_bc.*,
        ...
        if ($addCondition['status_posting']) {
            if ($addCondition['status_posting'] == "SUDAH POSTING") {
                $dataQry
                    ->where('inv_packing_bc.status_posting', 1);
            } else if ($addCondition['status_posting'] == "BELUM POSTING") {
                $dataQry
                    ->where('inv_packing_bc.status_posting', 0);
            }
        }

        if (isset($addCondition['search']) && !empty($addCondition['search'])) {
            $dataQry->groupStart()
                ->like('inv_packing_bc.no_container', $addCondition['search'])
                ->orLike('inv_packing_bc.no_seal', $addCondition['search'])
                ->orLike('inv_packing_bc.vessels_name', $addCondition['search'])
                ->groupEnd();
        }
