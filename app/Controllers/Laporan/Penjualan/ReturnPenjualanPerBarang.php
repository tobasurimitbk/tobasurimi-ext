...
class ReturnPenjualanPerBarang extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('Laporan/LaporanSales/LaporanReturnPerBarang/index', $data);
    }
    ...
    public function allTransaksi()
    {
        ...
        $dataSalesOrderReturn = $this->salesOrderReturnModel
            ->getAllSalesOrderReturnLokalBarang($condition, $addCondition, $pageSize, $offset);
        ...
        foreach ($dataSalesOrderReturn['data'] as $data) {
            if ($currentBarang !== $data->nama_barang) {
                if ($currentBarang !== null) {
                    array_push($dataAllSalesOrderReturn, [
                        "no" => '',
                        "id" => '',
                        "no_return" => 'Total Return: Rp ' . number_format(floatval($totalPerBarang)),
                        "no_dokumen" => '',
                        "tanggal_return" => '',
                        "note" => '',
                        "sum_qty_return" => '',
                ...
                $currentBarang = $data->nama_barang;
                $totalPerBarang = 0;

                array_push($dataAllSalesOrderReturn, [
                    "no" => '',
                    "id" => '',
                    "no_return" => $data->nama_barang,
                    "no_dokumen" => '',
                    "tanggal_return" => '',
                    "note" => '',
                    "sum_qty_return" => '',
            ...
            array_push($dataAllSalesOrderReturn, [
                ...
                "note" => $data->note,
                "sum_qty_return" => number_format(floatval($data->sum_qty_return)),
            ...
            $totalPerBarang += floatval($data->sum_amount_return);
        }
    ...
    public function printPDF($tglAwal, $tglAkhir, $filter, $search)
    {
        ...
        $dataSalesOrderReturn = $this->salesOrderReturnModel
            ->getAllSalesOrderReturnLokalBarang($condition, $addCondition, null, null);
        ...
        $dompdf->loadHtml(view('Laporan/LaporanSales/LaporanReturnPerBarang/print', $data));
    ...
    public function printExcel($tglAwal, $tglAkhir, $filter, $search)
    {
        ...
        $sheet->setCellValue('A2', 'LAPORAN RETURN PENJUALAN PER BARANG');
        ...
        $sheet->setCellValue('F5', 'Qty Return');
        $sheet->setCellValue('G5', 'Satuan');
        $sheet->setCellValue('H5', 'Jumlah');
        $sheet->setCellValue('I5', 'Nama Pelanggan');
        $sheet->setCellValue('J5', 'Nama Barang');
        $sheet->setCellValue('K5', 'Nama Sales');
        ...
        foreach ($dataAllSalesOrderReturn as $item) {
            if (isset($item['is_barang']) && $item['is_barang']) {
                ...
                $sheet->getStyle('A' . $row . ':K' . $row)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFF2F2F2');
            } else {
                ...
                $sheet->setCellValue('F' . $row, $item['sum_qty_return']);
                $sheet->setCellValue('G' . $row, $item['kode_satuan']);
                $sheet->setCellValue('H' . $row, $item['sum_amount_return']);
                $sheet->setCellValue('I' . $row, $item['nama_pelanggan']);
                $sheet->setCellValue('J' . $row, $item['nama_barang']);
                $sheet->setCellValue('K' . $row, $item['nama_sales']);
            }
            $row++;
        }
        ...
        $lastRow = $row - 1;
        $sheet->getStyle('A5:K' . $lastRow)->getBorders()->getAllBorders()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);

        $sheet->getStyle('F6:F' . $lastRow)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);
        $sheet->getStyle('H6:H' . $lastRow)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT);

        $filename = "Laporan_Return_Penjualan_Per_Barang_" . date('Ymd_His') . ".xlsx";
