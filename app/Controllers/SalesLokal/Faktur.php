...
use App\Models\SalesFakturDetailModel;
...
class Faktur extends BaseController
{
    ...
    protected $SalesFakturDetailModel;
    ...
    public function __construct()
    {
        ...
        $this->SalesFakturDetailModel = new SalesFakturDetailModel();
    ...
    public function index()
    {
        ...
        return view('SalesLokal/Faktur/index', $data);
    }
    ...
    public function createView()
    {
        ...
        return view('SalesLokal/Faktur/form', $data);
    }
    ...
    public function all()
    {
        ...
        if ($this->is_admin == '1') {
            $condition = [
                // "sales_faktur.id_company" => $this->this_company_id,
            ...
            $condition = [
                // "sales_faktur.id_company" => $this->this_company_id,
                "sales_faktur.deletedAt" => null,
                'sales_faktur.id_user' => $this->userId
            ];
        }
        ...
        $dataOrderForm = $this->SalesFakturModel
        ...
        foreach ($dataOrderForm['data'] as $data) {
            ...
            array_push($dataSalesOrder, [
                ...
                "qty_barang" => count($this->SalesFakturDetailModel->where('id_sales_order', $data->id)->where('deletedAt', null)->where('tipe_input', "order_form")->findAll()),
    ...
    public function save()
    {
        ...
        try {
            ...
            $dataSalesOrder =  $this->SalesFakturModel->insert($values);
            ...
            foreach ($items as $row) {
                ...
                $this->SalesFakturDetailModel->save($valueBarang);
            }

            $this->SalesFakturModel->update($dataSalesOrder, ['qty_barang' => $totalQty]);

            $this->SalesFakturModel->db->transComplete();
    ...
    {
        $id = decrypt($id);
        $dataSalesOrder = $this->SalesFakturModel->getSalesOrderLokalById(($id));
    ...
    public function getByCustomerId($customerId)
    {
        ...
        $dataSalesOrder = $this->SalesFakturModel->asObject()
            ->select($select)
            ->where($condition)
            ->findAll();
        ...
        try {
            ...
            foreach ($items as $row) {
                ...
                if ($row->id == 0 || $row->id == null || $row->id == "") {
                    ...
                    $this->SalesFakturDetailModel->update($row->id, $valueBarang);
                }
            }

            $this->SalesFakturModel->update(
                $id,
                [
                    "no_sales_order"        => strtoupper($postData['no_sales_order']),
    ...
    public function delete()
    {
        try {
            ...
            if (!empty($id)) {
                ...
                $this->SalesFakturModel->delete($id);
                $dataDetail = $this->SalesFakturDetailModel->where('id_sales_order', $id)->findAll();

                foreach ($dataDetail as $item) {
                    $this->SalesFakturDetailModel->delete($item['id']);
                }
    ...
    public function deleteOrderForm()
    {
        ...
        $getBarangSalesOrderDetail = $this->SalesFakturDetailModel->select('id_sales_order, id_barang')->where('id', $id)->first();
        ...
        $this->SalesFakturDetailModel->delete($id);

        $getAllBarangSalesOrderDetail = $this->SalesFakturDetailModel->select('qty, harga_barang, discount_percentage, amount')->where('id_sales_order', $getBarangSalesOrderDetail['id_sales_order'])->where('id_barang !=', 85)->findAll();
        ...
        $this->SalesFakturModel->update($getBarangSalesOrderDetail['id_sales_order'], ['qty_barang' => ($totalQty * 2), 'total_harga' => $total_harga]);
    ...
    {
        $SOData = $this->SalesFakturModel->asObject()
            ->select('tax_status, include_pa')
            ->find($id);
        $itemList = $this->SalesFakturDetailModel->getItemListByIds($id);
    ...
    public function getItemListbyIds()
    {
        ...
        $datas = $this->SalesFakturDetailModel->getItemListByIds($ids);
    ...
    public function printOrder($id)
    {
        ...
        $salesOrderData = $this->SalesFakturModel->asObject()
            ->select($soSelectQry)
            ->join('customers', 'customers.id = sales_faktur.id_customer', 'left')
            ->join('metadata', 'metadata.id = sales_faktur.payment_terms', 'left')
    ...
    public function HistoriHargaBarang()
    {
        ...
        $id_user = $this->userId;
        $listHargaBarang = [];
        $dataSalesOrder = $this->SalesFakturModel
            ->join('customers', 'customers.id = sales_faktur.id_customer', 'left')
            ->find($id);
        $listBarang = $this->SalesFakturDetailModel
            ->select('sales_faktur_detail.id AS id_sales_order_detail, sales_faktur_detail.*, barang_master_sales.barang_name AS nama_barang, barang_master_sales.kode_barang')
            ->join('barang_master_sales', 'barang_master_sales.id = sales_faktur_detail.id_barang', 'left')
            ->where('sales_faktur_detail.id_sales_order', $id)
            ->where('sales_faktur_detail.deletedAt', null)
            ->findAll();

        foreach ($listBarang as $key => $value) {
            $listBarang2 = $this->SalesFakturDetailModel
                ->select('sales_faktur_detail.id AS id_sales_order_detail, sales_faktur_detail.*, barang_master_sales.barang_name AS nama_barang, barang_master_sales.kode_barang')
                ->join('sales_faktur', 'sales_faktur.id = sales_faktur_detail.id_sales_order', 'left')
                ->join('barang_master_sales', 'barang_master_sales.id = sales_faktur_detail.id_barang', 'left')
                ->where('sales_faktur.id_user', $id_user)
                ->where('sales_faktur.id_customer', $dataSalesOrder['id_customer'])
                ->where('sales_faktur_detail.id_barang', $value['id_barang'])
                ->where('sales_faktur_detail.deletedAt', null)
                ->findAll();
    ...
    public function posting()
    {
        try {
            ...
            $this->SalesFakturModel->update($id, $payload);
