...
class Attendances extends BaseController
{
    ...
    // public function sync_attendance_backup()
    // {
    ...
    public function get_sync_attendance($attendanceUnitId)
    {
        $resAttendanceUnit = $this->AttendancesUnitModel
            ->where('id', $attendanceUnitId)
            ->first();

        $ip = $resAttendanceUnit['ip'];
        $unitKey = $resAttendanceUnit['unit_key'];
        $companyId = $resAttendanceUnit['company_id'];
        $dataAttendance = array();

        if (icmpPing($ip, 1)) {
            // Cari Data Finger
            $resFinger = $this->get_data_finger($ip, $unitKey);

            for ($j = 0; $j < count($resFinger); $j++) {
                $employeeId = $resFinger[$j]["id"];
                $date = $resFinger[$j]["date"];

                array_push($dataAttendance, [
    ...
    public function get_data_finger($ip, $unit_key)
    {
        $Connect = fsockopen($ip, "80", $errno, $errstr, 1);
        if ($Connect) {
            $soap_request = "<GetAttLog><ArgComKey xsi:type=\"xsd:integer\">" . $unit_key . "</ArgComKey><Arg><PIN xsi:type=\"xsd:integer\">All</PIN></Arg></GetAttLog>";
            $newLine = "\r\n";
