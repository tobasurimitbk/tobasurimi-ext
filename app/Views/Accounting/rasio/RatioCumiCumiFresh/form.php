...
<section class="section">
    <div class="section-header">
        <h1><?= empty($rasio) ? "Tambah Ratio Cumi Cumi Fresh" : "Update Ratio Cumi Cumi Fresh" ?></h1>
        <div class="col-button-tambah-spp text-right">
            <a class="btn btn-hide-form btn-discard" href="<?= base_url("ratio-cumi-cumi-fresh"); ?>">
                Kembali
            </a>
...
<script>
    ...
    const getDataJurnalSubsidi = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && coa_id) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-cumi-cumi-fresh/get-jurnal'); ?>`,
                method: "GET",
                data: {
    ...
    const getDataRawMaterialI = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-cumi-cumi-fresh/get-material-i'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    const getDataRawMaterialII = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori && akun_pemakaian.length > 0) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-cumi-cumi-fresh/get-barang-digunakan-penolong'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
            ...
            $.ajax({
                url: `<?= base_url('ratio-cumi-cumi-fresh/get-barang-jadi'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    const getDataCost = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-cumi-cumi-fresh/get-cost'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    $(document).ready(function() {
        ...
        function loadContent(page) {
            $.ajax({
                url: '<?= base_url('/ratio-cumi-cumi-fresh/load_content') ?>',
                type: 'GET',
                data: {
                    page: page
                },
                ...
                if (!isValid) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            if (id) {
                                $.ajax({
                                    url: "<?= base_url("ratio-cumi-cumi-fresh/update"); ?>",
                                    data: data,
                                    ...
                                    success: function(response) {
                                        ...
                                        if (response.status) {
                                            Swal.fire({
                                                ...
                                                .then(() => {
                                                    window.location.href = "<?= base_url("ratio-cumi-cumi-fresh"); ?>";
                                                })
                                        } else {
                                            Swal.fire({
                                                icon: 'error',
                            ...
                            // CREATE
                                ...
                                $.ajax({
                                    url: "<?= base_url("ratio-cumi-cumi-fresh/save"); ?>",
                                    data: data,
                                    beforeSend: function(xhr) {
                                        xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                        setLoading();
