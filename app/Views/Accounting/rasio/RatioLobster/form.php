...
<section class="section">
    <div class="section-header">
        <h1><?= empty($rasio) ? "Tambah Ratio Lobster" : "Update Ratio Lobster" ?></h1>
        <div class="col-button-tambah-spp text-right">
            <a class="btn btn-hide-form btn-discard" href="<?= base_url("ratio-lobster"); ?>">
                Kembali
            </a>
...
<script>
    ...
    const getDataJurnalSubsidi = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && coa_id) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-lobster/get-jurnal'); ?>`,
                method: "GET",
                data: {
    ...
    const getDataRawMaterialI = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-lobster/get-material-i'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    const getDataRawMaterialII = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori && akun_pemakaian.length > 0) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-lobster/get-barang-digunakan-penolong'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
            ...
            $.ajax({
                url: `<?= base_url('ratio-lobster/get-barang-jadi'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    const getDataCost = function() {
        ...
        if (department_id && tanggal_awal && tanggal_akhir && kategori) {
            setLoading();
            $.ajax({
                url: `<?= base_url('ratio-lobster/get-cost'); ?>`,
                method: "GET",
                data: {
                    department: department_id,
                    tanggal_awal: tanggal_awal,
    ...
    $(document).ready(function() {
        ...
        function loadContent(page) {
            $.ajax({
                url: '<?= base_url('/ratio-lobster/load_content') ?>',
                type: 'GET',
                data: {
                    page: page
                },
                ...
                if (!isValid) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            if (id) {
                                $.ajax({
                                    url: "<?= base_url("ratio-lobster/update"); ?>",
                                    data: data,
                                    ...
                                    success: function(response) {
                                        ...
                                        if (response.status) {
                                            Swal.fire({
                                                ...
                                                .then(() => {
                                                    window.location.href = "<?= base_url("ratio-lobster"); ?>";
                                                })
                                        } else {
                                            Swal.fire({
                                                icon: 'error',
                            ...
                            // CREATE
                                ...
                                $.ajax({
                                    url: "<?= base_url("ratio-lobster/save"); ?>",
                                    data: data,
                                    beforeSend: function(xhr) {
                                        xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                        setLoading();
