    ...
    <div class="modal-dialog" style="min-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><label class="title-name"></label> Mesin Finger</h5>
            </div>
...
<script>
    ...
    function pingAction() {
        ...
        $.ajax({
            url: "<?= base_url("attendances-unit/ping"); ?>",
            beforeSend: function(xhr) {
                setLoading();
                xhr.setRequestHeader('X-CSRF-Token', csrf.val());
            },
        ...
        if ($(".create-form").valid()) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let id = $(".id").val();
                    let url = id == '' ? "<?= base_url("attendances-unit/save"); ?>" : "<?= base_url("attendances-unit/update"); ?>";
                    $.ajax({
                        url: url,
                        data: data,
                        beforeSend: function(xhr) {
