                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="input-group">
                            ...
                            <?php if (!empty($dataBiayaImporList)): ?>
                    ...
                    <div class="col-sm-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            ...
                            <label for="floatingInput">Pilih Tipe PO (Opsional)</label>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <select <?= !empty($dataBiayaImpor) ? ($dataBiayaImpor['status_bayar'] == 1 ? 'disabled' : '') : '' ?> class="form-select po_id" name="po_id" id="po_id">
                                ...
                                <?php if (!empty($dataPoDetail)): ?>
                                    <?php if ($dataPoDetail != null): ?>
                                        <option selected value="<?= $dataPoDetail['id'] ?>"><?= $dataPoDetail['po_no'] ?></option>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </select>
                            <label for="floatingInput">Pilih Nomor PO (Opsional)</label>
                        </div>
                    </div>
...
<div class="modal detail-modal" id="detailBarang" tabindex="1">
    <div class="modal-dialog" style="min-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title title-secondary"><label id="label-detail-barang"></label> Barang Impor</h5>
            </div>
                            ...
                            <div class="form-floating mb-3" style="height: 50px;">
                                <select class="form-select spesifikasi_id" name="spesifikasi_id" id="spesifikasi_id">
                                    ...
                                    <?php foreach ($dataBarang as $d): ?>
                                        <option data-kode_barang="<?= $d['kode_barang'] ?>" data-barang_name="<?= str_replace(['"', "'"], '', $d['barang_name_master'] . " - " . $d['spesifikasi'])  ?>" value="<?= $d['barang_master_spesifikasi_id'] ?>"><?= "(" . $d['kode_barang'] . ") " . $d['barang_name_master'] . " - " . $d['spesifikasi'] ?></option>
                                    <?php endforeach; ?>
                                </select>
...
<?php if (!empty($dataBiayaImporList)): ?>
    <div class="modal detail-modal" id="copyDataModal" tabindex="1">
        <div class="modal-dialog" style="min-width: 900px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title title-secondary">Copy Biaya Impor</h5>
                </div>
                <form id="form-copy-biaya" role="form" method="POST">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <select class="form-select biaya_impor_id_copy" name="biaya_impor_id_copy" id="biaya_impor_id_copy">
                                        <option value=""></option>
                                        <?php foreach ($dataBiayaImporList as $d): ?>
...
<script>
    ...
    $(document).ready(function() {
        <?php if (!empty($dataBiayaImpor)) { ?>
            ...
            <?php foreach ($dataDetailBarang as $d): ?>
                listBarang.push({
                    ...
                    barang_name: "<?= $d['barang_name'] . " " . $d['spesifikasi'] ?>",
                    valas_id: "<?= $d['valas_id'] ?>",
                    valas_name: "<?= $d['valas_name'] ?>",
            ...
            drawTableBarangImport(listBarang);
        ...
        $('.biaya_impor_id_copy,.po_id, .divisi_id, .vendor_pelayaran_id, .tax_id, .tax_status, .supplier_id, .tipe_po, .valas_id,.spesifikasi_id,.satuan_id,.valas_barang_id')
            .parent('div')
            .children('span')
            .children('span')
            .children('span')
        ...
        $('.biaya_impor_id_copy,.po_id, .divisi_id, .vendor_pelayaran_id, .tax_id, .tax_status, .supplier_id, .tipe_po, .valas_id.valas_id,.spesifikasi_id,.satuan_id,.valas_barang_id')
            .parent('div')
            .children('span')
            .children('span')
            .children('span')
        ...
        $('#btnSubmiCopyData').click(function(e) {
            ...
            if ($('#form-copy-biaya').valid()) {
                // Ajax Get Data
                var id = $('#biaya_impor_id_copy option:selected').val();
                getCopyData(id);
            }
        })
        ...
        $('#btnCopyDataModal').click(function(e) {
            e.preventDefault();
            $('#biaya_impor_id_copy').val(null).change();
        ...
        $(".btn-submit-parent").click(function() {
            if (listBiayaImpor.length === 0) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Wajib mengisikan barang impor !',
                    confirmButtonColor: '#4e73df',
                })
            } else {
                var id = $('#id').val();
            ...
            success: function(res) {
                if (res.status) {
                    if (res.data != null) {
                        ...
                        $('#tipe_po').val(dataPo.tipe_po).change();
                        $('#txt_valas').text("(" + dataPo.valas_name + ")");


                        listBarang = [];
                        $.each(dataPoDetail, function(i, v) {
                            listBarang.push({
                                id_detail_barang: v.id,
                                spesifikasi_id: v.spesifikasi_id,
                                kode_barang: v.kode_barang,
                                barang_name: v.barang_name + " " + v.spesifikasi,
                                valas_id: dataPo.valas_id,
                                valas_name: dataPo.valas_name,
                                qty_barang: v.qty,
    ...
    function getCopyData(id) {
        $.ajax({
            url: `<?= base_url("biaya-impor/get-data-biaya"); ?>`,
            method: "GET",
            dataType: "json",
            data: {
                id: id,
            ...
            success: function(res) {
                if (res.status) {
                    if (res.data != null) {
                        var dataBiayaImpor = res.data.dataBiayaImpor;
                        var dataDetailBarang = res.data.dataDetailBarang;
                        var dataContainer = res.data.dataContainer;

                        $('#auto_generate').prop('checked', false);
                        $('#no_invoice').attr('readonly', false);
                        // DATA BIAYA IMPOR
                        $('#no_invoice').val(dataBiayaImpor.no_invoice);
                        $('#tanggal_invoice').val(dataBiayaImpor.tanggal_invoice);
                        $('#divisi_id').val(dataBiayaImpor.divisi_id).change();
                        $('#supplier_id').val(dataBiayaImpor.supplier_id).change();
                        $('#no_bl').val(dataBiayaImpor.no_bl).change();
                        $('#tipe_po').val(dataBiayaImpor.tipe_po).change();
                        $('#po_id').val(dataBiayaImpor.po_id).change();
                        $('#vendor_pelayaran_id').val(dataBiayaImpor.vendor_pelayaran_id).change();
                        $('#shipper_prev').val(dataBiayaImpor.shipper);
                        $('#consigne_prev').val(dataBiayaImpor.consigne);
                        $('#port_of_origin_prev').val(dataBiayaImpor.port_of_origin);
                        $('#port_of_destination_prev').val(dataBiayaImpor.port_of_destination);
                        // DETAIL BARANG
                        ...
                        // CONTAINER
                        $.each(dataContainer, function(i, v) {
                            listContainer.push({
                                id_container: v.id,
                                no_container: v.no_container,
                                detail_container: v.detail_container
                            });
                        });
    ...
    $('#btnSubmitDetailBarang').click(function(e) {
        ...
        if ($('#form-detail-barang').valid()) {
            ...
            var kodeBarang = $('#spesifikasi_id option:selected').data('kode_barang');
            var barangName = $('#spesifikasi_id option:selected').data('barang_name');
            var valasId = $('#valas_barang_id option:selected').val();
            var valasName = $('#valas_barang_id option:selected').data('valas_name');
            ...
            if (idBarangDetail) {
                ...
                listBarang[index].spesifikasi_id = spesifikasiId;
    ...
    function drawTableBarangImport(listBarang) {
    ...
    function detailRowBarang(id_detail_barang) {
        ...
        $('#valas_barang_id').val(item.valas_id).change();
