...
<section class="section">
    <div class="section-header">
        <h1 class="title-name"><?= !empty($dataBiayaLokal) ? "Update Biaya Lokal" : "Tambah Biaya Lokal" ?></h1>
        <div class="col-button-tambah-spp">
            <a class="btn btn-hide-form btn-discard float-right" href="<?= base_url("biaya-lokal"); ?>">
                Kembali
            </a>
            <?php if (!empty($dataBiayaLokal)) { ?>
                <?php if (can('Biaya Exim', 'Biaya Lokal', 'p')): ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= encrypt($dataBiayaLokal['id']) ?>')">
                        Print
                    </button>
                <?php endif; ?>
    ...
    <div class="card">
        <div class="card-body">
            <form class="create-form form-add-spp" role="form" method="POST" enctype="multipart/form-data" id="form-parent">
                ...
                <input autocomplete="one-time-code" value="<?= !empty($dataBiayaLokal) ? encrypt($dataBiayaLokal['id']) : '' ?>" type="hidden" class="id" name="id" id="id" />
                <?= csrf_field() ?>
                <div class="row">
                    <div class="col-md-4">
                    ...
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                        ...
                        <div class="row mb-3">
                            <div class="table-responsive">
                                <table class="table table-bordered nowrap table-hover-tobasurimi dataTable" id="biayaLokalDetailTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody class="body-biaya-lokal-detail" id="body-biaya-lokal-detail">

                                    </tbody>
                                    <tfoot class="foot-biaya-lokal-detail" id="foot-biaya-lokal-detail">
                                        <tr>
    ...
    <div class="modal-dialog" style="min-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title title-secondary"><label id="label-biaya-lokal"></label> Biaya Impor</h5>
            </div>
            <form id="form-biaya-lokal" role="form" method="POST">
                <input type="hidden" name="id_biaya_lokal_detail" id="id_biaya_lokal_detail">
    ...
    <div class="modal-dialog" style="min-width: 900px;">
        <div class="modal-content">
            ...
            <form id="form-pengenaan-pajak" role="form" method="POST">
                <input type="hidden" name="id_biaya_lokal_pajak" id="id_biaya_lokal_pajak">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="input-group input-group-password">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.divisi_id, .vendor_pelayaran_id, .tax_id, .tax_status, .customer_id,.barang_id,.valas_id,.satuan_id')
            .parent('div')
            .children('span')
            .children('span')
            .children('span')
        ...
        var validatorBiayaLokal = $("#form-biaya-lokal").validate({
            rules: {
                uraian_biaya: {
                    required: true
                },
        ...
        $('#btnSubmitDetailBiaya').click(function(e) {
            e.preventDefault();
            if ($('#form-biaya-lokal').valid()) {
                var idBiayaLokalDetail = $('#id_biaya_lokal_detail').val();
                ...
                console.log(idBiayaLokalDetail);

                if (idBiayaLokalDetail) {
                    // UPDATE
                    var index = null;
                    for (var i = 0; i < listBiayaLokal.length; i++) {
                        if (listBiayaLokal[i].id_biaya_lokal_detail == idBiayaLokalDetail) {
                            index = i;
                            break;
                        }
                    }

                    listBiayaLokal[index].uraian_biaya = uraianBiaya;
                    listBiayaLokal[index].valas_id = valasId;
                    listBiayaLokal[index].valas_name = valasName;
                    listBiayaLokal[index].nilai_biaya = parseFloat(nilaiBiaya);
                    listBiayaLokal[index].nilai_exchange_rate = parseFloat(nilaiExchangeRate);
                    listBiayaLokal[index].nilai_biaya_idr = parseFloat(nilaiBiayaIdr);

                } else {
                    // CREATE
                    listBiayaLokal.push({
                        id_biaya_lokal_detail: getID(),
        ...
        $('#btnSubmitPengenaanPajak').click(function(e) {
            ...
            if ($('#form-pengenaan-pajak').valid()) {
                var idBiayaLokalPajak = $('#id_biaya_lokal_pajak').val();
                ...
                if (idBiayaLokalPajak) {
                    // UPDATE
                    var index = null;
                    for (var i = 0; i < listPajak.length; i++) {
                        if (listPajak[i].id_biaya_lokal_pajak == idBiayaLokalPajak) {
                            index = i;
                            break;
                        }
                    }
                    ...
                    // Create
                    idBiayaLokalPajak = getID();
                    listPajak.push({
                        id_biaya_lokal_pajak: idBiayaLokalPajak,
        ...
        $(".btn-submit-parent").click(function() {
            if (listBiayaLokal.length === 0) {
                Swal.fire({
                    icon: 'error',
                    title: 'Wajib mengisikan biaya lokal !',
                ...
                if ($("#form-parent").valid()) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            let id = $('#id').val();
                            let url = id == '' ? "<?= base_url('biaya-lokal/create') ?>" : "<?= base_url('biaya-lokal/update') ?>";
                            ...
                            data.append("listBiayaLokal", JSON.stringify(listBiayaLokal));
                                ...
                                success: function(response) {
                                    if (response.status) {
                                        Swal.fire({
                                            ...
                                            .then(() => {
                                                window.location.href = "<?= base_url("biaya-lokal") ?>";
                                            })
                                    } else {
                                        Swal.fire({
                                            icon: 'error',
    ...
    function drawTableBiayaLokal(listBiayaLokal) {
        const table = $('#biayaLokalDetailTable');
        const tbody = table.find('#body-biaya-lokal-detail');
        const tfoot = table.find('#foot-biaya-lokal-detail');
                ...
                <tr style="color:whitesmoke;">
                    ...
                    <td class="text-center">
                        ...
                        <?php else : ?>
                            ...
                            <button type="button" class="btn btn-danger" onclick="deleteRowBiayaLokal('${item.id_biaya_lokal_detail}')">
                ...
                newRow.append($('<td>').html(
                    <?php if (!empty($dataBiayaLokal)) : ?> <?php if ($dataBiayaLokal['status_bayar'] == 1) : ?> `-`
                        ...
                        <button type="button" class="btn btn-warning posting-spp mr-1" onclick="detailRowPajak('${item.id_biaya_lokal_pajak}')">
                            ...
                            <button type="button" class="btn btn-danger" onclick="deleteRowPajak('${item.id_biaya_lokal_pajak}')">
                    ...
                    <?php else : ?> `
                        <button type="button" class="btn btn-warning posting-spp mr-1" onclick="detailRowPajak('${item.id_biaya_lokal_pajak}')">
                                <i class="fa fa-pencil fa-sm" aria-hidden="true"></i>
                            </button><button type="button" class="btn btn-danger" onclick="deleteRowPajak('${item.id_biaya_lokal_pajak}')">
                                <i class="fa fa-trash fa-sm" aria-hidden="true"></i>
                            </button>
                    `
                    <?php endif; ?>
    ...
    function detailRowBiayaLokal(id_biaya_lokal_detail) {
        var item = null;
        for (var i = 0; i < listBiayaLokal.length; i++) {
            if (listBiayaLokal[i].id_biaya_lokal_detail == id_biaya_lokal_detail) {
                item = listBiayaLokal[i];
                break;
            }
        }

        $('#id_biaya_lokal_detail').val(item.id_biaya_lokal_detail);
        ...
        $('#label-biaya-lokal').text("Update ");
        $('#detailBiayaModal').modal('show');
    }

    function detailRowPajak(id_biaya_lokal_pajak) {
        var item = null;
        for (var i = 0; i < listPajak.length; i++) {
            if (listPajak[i].id_biaya_lokal_pajak == id_biaya_lokal_pajak) {
                item = listPajak[i];
                break;
            }
        }

        $('#id_biaya_lokal_pajak').val(item.id_biaya_lokal_pajak);
    ...
    function deleteRowPajak(id_biaya_lokal_pajak) {
        var indexToRemove = -1;
        for (var i = 0; i < listPajak.length; i++) {
            if (listPajak[i].id_biaya_lokal_pajak == id_biaya_lokal_pajak) {
                indexToRemove = i;
    ...
    function deleteRowBiayaLokal(id_biaya_lokal_detail) {
        var indexToRemove = -1;
        for (var i = 0; i < listBiayaLokal.length; i++) {
            if (listBiayaLokal[i].id_biaya_lokal_detail == id_biaya_lokal_detail) {
                indexToRemove = i;
        ...
        if (indexToRemove !== -1) {
            listBiayaLokal.splice(indexToRemove, 1);
        }
        drawTableBiayaLokal(listBiayaLokal);
        calculateTotalBiayaAndTax();
    }
    ...
    function calculateTotalBiayaAndTax() {
        ...
        $.each(listBiayaLokal, function(i, v) {
    ...
    const print = function(id) {
        window.open("<?= base_url('biaya-lokal/print') ?>" + '/' + id, "_blank");
    }
                    ...
                    success: function(response) {
                        if (response.status) {
                            Swal.fire({
                                ...
                                .then(() => {
                                    location.href = "<?= base_url('biaya-lokal') ?>";
                                })
                        }
                    },
                });
