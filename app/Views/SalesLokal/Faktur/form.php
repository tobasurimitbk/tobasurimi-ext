...
<script>
    ...
    $(document).ready(function() {
            ...
            if (dataId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "<?= base_url("faktur-sales/delete-detail"); ?>",
                            data: {
                                id: dataId,
                            },
        ...
        $("#hidden_tipe_pelanggan").on('input change keyup paste', function() {
            ...
            $.ajax({
                url: "<?= base_url('/faktur-sales/getmetaData'); ?>" + "/" + tipePelanggan,
                method: "GET",
                dataType: "json",
        ...
        var validator = $(".create-form").validate({
            ...
            messages: {
                ...
                id_customer: {
                    required: "Nama Supplier Lokal wajib diisi"
                },
                order_date: {
        ...
        $(".id_barang").change(function() {
            if ($(".id_barang").val()) {
                ...
                let hargaPokok = $(".id_barang option:selected").data("harga_pokok") ? $(".id_barang option:selected").data("harga_pokok") : 0;
                ...
                $(".warehouse").on('change', function() {
                    ...
                    $(".stok").val("");
                    $.ajax({
                        url: "<?= base_url('/faktur-sales/stok'); ?>" + "/" + idBarang + "/" + warehouseId,
                        method: "GET",
                        dataType: "json",
                        success: function(res) {
                ...
                $(".satuan").val(satuan);
                $(".harga").val(greatFormatRupiah(hargaPokok));
            ...
            if ($(".create-form").valid()) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (id) {
                            $.ajax({
                                url: "<?= base_url("faktur-sales/update"); ?>",
                                data: data,
                                beforeSend: function(xhr) {
                                    xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                    setLoading();
                                ...
                                success: function(response) {
                                    ...
                                    if (response.status) {
                                        ...
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed) {
                                                // Cetak
                                                window.location.href = `<?= base_url("faktur-sales/print"); ?>/${response.id}`;
                                            } else if (result.isDenied) {
                                                // Buat baru
                                                window.location.reload();
                                            } else if (result.dismiss === Swal.DismissReason.cancel) {
                                                // Tutup - redirect to faktur-sales
                                                window.location.href = `<?= base_url("faktur-sales"); ?>`;
                                            }
                                        });
                                    } else {
                                        Swal.fire({
                        ...
                        // CREATE
                            ...
                            $.ajax({
                                url: "<?= base_url("faktur-sales/save"); ?>",
                                data: data,
                                beforeSend: function(xhr) {
                                    xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                    setLoading();
    ...
    function changeStatus() {
        ...
        if (value) {
            $.ajax({
                url: "<?= base_url("faktur-sales/generate-no-order-form"); ?>",
                method: "POST",
                dataType: "json",
                beforeSend: function(xhr) {
                    xhr.setRequestHeader('X-CSRF-Token', csrf.val());
    ...
    function getListCustomer() {
        $.ajax({
            url: `<?= base_url('faktur-sales/customer'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function checkPiutang(id, callback) {
        if (id) {
            $.ajax({
                url: `<?= base_url("faktur-sales/check-piutang"); ?>/${id}`,
                method: "GET",
                beforeSend: function() {
                    setLoading();
                },
