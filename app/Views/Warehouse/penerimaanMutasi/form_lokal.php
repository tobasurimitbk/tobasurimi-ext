                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <div class="input-group input-group-password">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <input autocomplete="one-time-code" <?= !empty($penerimaanMutasi) ? ($penerimaanMutasi['status_posting'] == "1" ? "disabled" : '') : ''; ?> value="<?= !empty($penerimaanMutasi) ? $penerimaanMutasi['penerimaan_mutasi_no'] : "" ?>" type="text" class="form-control penerimaan_mutasi_no" id="penerimaan_mutasi_no" name="penerimaan_mutasi_no" placeholder="No. Penerimaan Mutasi">
                    ...
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <select <?= !empty($penerimaanMutasi) ?  ($penerimaanMutasi['status_posting'] == "1" ? "disabled" : '') : '' ?> class="form-select divisi_id" id="divisi_id" name="divisi_id" aria-label="Floating label select example">
                                <option value=""></option>
                    ...
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <select <?= !empty($penerimaanMutasi) ?  ($penerimaanMutasi['status_posting'] == "1" ? "disabled" : '') : ''; ?> multiple class="form-select multiple_mutasi_id" name="multiple_mutasi_id[]" id="multiple_mutasi_id">
                                <option value=""></option>
                                <?php if (!empty($penerimaanMutasi)) : ?>
                        ...
                        <table class="table table-bordered nowrap table-hover-tobasurimi dataTable" id="dataTable" width="100%" cellspacing="0">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th>Barang</th>
                                    <th>Spesifikasi</th>
                                    <th>Qty Sisa Mutasi</th>
                                    <th>Satuan Mutasi</th>
                                    <th>Qty Diterima Sekarang</th>
                                    <th>Qty Diterima Total</th>
...
<script>
    ...
    function drawTable(listBarang) {
        ...
        if (listBarang.length === 0) {
            const newRow = $('<tr>');
            newRow.append($('<td colspan="16" class="text-left">Tidak Ada Barang</td>'));
        ...
        $.each(listBarang, function(i, v) {
            ...
            <tr style="color:whitesmoke;">
                <td>${no++}</td>
                <td>${v.divisi_asal}</td>
                <td>${v.warehouse_asal}</td>
                <td>${v.divisi_tujuan}</td>
                <td>${v.warehouse_tujuan}</td>
                <td>${v.no_mutasi}</td>
                <td>${v.supplier_name}</td>
                <td>${v.kode_barang}</td>
                <td>${v.barang_name}</td>
                <td>${v.spesifikasi}</td>
                <td class="qty_sisa_mutasi text-end">${greatFormatRupiah(parseFloat(v.qty_sisa_mutasi).toFixed(2))}</td>
                <td>${v.satuan_mutasi}</td>
                <td>
                    <input 
                        class="form-control qty_diterima_sekarang" 
                        style="height: 40px; padding-bottom: 10px;" 
                        autocomplete="off"
                        data-mutasi_detail_id="${v.mutasi_detail_id}" 
                        oninput="this.value = greatFormatRupiah(this.value)"
                        type="text" 
                        value="${greatFormatRupiah(parseFloat(v.qty_diterima_sekarang).toFixed(2))}"
                    >
                </td>
                <td class="qty_diterima_total text-end">${greatFormatRupiah(parseFloat(v.qty_diterima_total).toFixed(2))}</td>
                <td>${v.satuan_diterima}</td>
                <td>
                    <button type="button" 
                        class="btn btn-danger" 
                        onclick="deleteDetail(${v.mutasi_detail_id})">
        ...
        // Setelah tabel digambar, tambahkan event listener langsung
        $('.qty_diterima_sekarang').on('input', function() {
            calculateQtyDiterima();
        });
    }
    ...
    function deleteDetail(id) {
        ...
        for (var i = 0; i < listBarang.length; i++) {
            if (Number(listBarang[i].mutasi_detail_id) == id) {
                indexToRemove = i;
                break;
            }
        }
        ...
        removeMutasiList(listBarang);
        drawTable(listBarang);
    }

    function removeMutasiList(listBarang) {
        var mutasiIdList = [];
        for (let i = 0; i < listBarang.length; i++) {
            mutasiIdList.push(listBarang[i].mutasi_id);
        }

        var unique = [...new Set(listBarang.map(item => item.mutasi_id))];
        $('#multiple_mutasi_id').val(unique).trigger('change.select2');
    }

    function calculateQtyDiterima() {
        $('.qty_diterima_sekarang').each(function() {
            const $row = $(this).closest('tr');
            const mutasiDetailId = $(this).data('mutasi_detail_id');

            // Ambil data dari tabel
            let qtySisaMutasi = destroyFormatRupiah($row.find('.qty_sisa_mutasi').text());
            let qtyDiterimaSekarang = destroyFormatRupiah($(this).val()) || 0;

            // Ambil data dari array (asli, bukan hasil render)
            let item = listBarang.find(x => x.mutasi_detail_id == mutasiDetailId);
            if (!item) return;

            const qtySudahDiterimaSebelumnya = item.qty_diterima_total - item.qty_diterima_sekarang; // total lama sebelum edit

            // Validasi agar tidak melebihi sisa mutasi
            if (qtyDiterimaSekarang > item.qty_sisa_mutasi + item.qty_diterima_sekarang) {
                qtyDiterimaSekarang = item.qty_sisa_mutasi + item.qty_diterima_sekarang;
                $(this).val(greatFormatRupiah(qtyDiterimaSekarang));
            }

            // Hitung ulang total & sisa
            const qtyDiterimaTotalBaru = qtySudahDiterimaSebelumnya + qtyDiterimaSekarang;
            const sisaBaru = Math.max(item.qty_sisa_mutasi + item.qty_diterima_sekarang - qtyDiterimaSekarang, 0);

            // Update tampilan tabel
            $row.find('.qty_diterima_total').text(greatFormatRupiah(qtyDiterimaTotalBaru));
            $row.find('.qty_sisa_mutasi').text(greatFormatRupiah(sisaBaru));

            // Update data di array
            item.qty_diterima_sekarang = qtyDiterimaSekarang;
            item.qty_diterima_total = qtyDiterimaTotalBaru;
            item.qty_sisa_mutasi = sisaBaru;
        });
    }
    ...
    <?php if (!empty($penerimaanMutasi)) : ?>
        ...
        //     }
        // })
        listBarang = <?= json_encode($penerimaanMutasiDetail) ?>;
        drawTable(listBarang);
    <?php else: ?>
        changeStatus();
    <?php endif; ?>
