                ...
                <table class="table table-bordered nowrap table-hover-tobasurimi dataTable" id="dataTable" width="100%" cellspacing="0">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th onclick="changeSort('type_barang_sales')">Tipe Barang Sales</th>
                            <th onclick="changeSort('barang_sales_name')">Barang Sales</th>
                            <th onclick="changeSort('barang_inventori_name')">Barang Inventori</th>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-floating mb-3" style="height: 50px;">
                                <input autocomplete="one-time-code" type="text" class="form-control barang_sales_name" id="barang_sales_name" name="barang_sales_name" placeholder="Nama Barang Sales">
                                <label for="floatingInput">Nama Barang Sales</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-floating mb-3" style="height: 50px;">
                                <select class="form-select barang_master_id" name="barang_master_id" id="barang_master_id">
                                    <option value=""></option>
                                </select>
                                <label for="floatingInput">Pilih Barang Inventori</label>
                            </div>
                        </div>
                    </div>
                </form>
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("alias-barang/all-sales"); ?>",
            dataSrc: "data",
            data: function(data) {
        ...
        columns: [{
            ...
            {
                data: "type_barang_sales",
                className: "text-left"
            },
            {
                data: "barang_sales_name",
                className: "text-left"
            },
            {
                data: "barang_inventori_name",
                className: "text-left"
            },
            {
                data: "id",
                ...
                render: function(data, type, row) {
                    let id = row.id;
                    let barang_sales_name = row.barang_sales_name;
                    return `
                        <div class="mt-0 actions">
                            <?php if (can('Inventori', 'Alias Barang', 'u')): ?>
                                <a href="javascript:void(0)" onclick="edit('${id}','${barang_sales_name}')" data-toggle="tooltip" title="Edit" class="btn btn-primary">
    ...
    $('#barang_master_id').select2({
        placeholder: "Cari Kode / Nama Barang Inventori",
        theme: "bootstrap-5",
        allowClear: true,
        dropdownParent: $('#updateAliasModal'),
        ajax: {
            ...
            data: function(params) {
                return {
                    q: params.term,
                    type_barang: "bahan_jadi" // ambil barang jadi
                };
            },
    ...
    $("#barang_master_id")
        ...
        .children('span')
        .children('span')
        .css('margin-top', '20px').css('margin-left', '-7px');
    ...
    var validator = $("#update-alias-form").validate({
        ...
        messages: {
            barang_master_id: {
                required: "pilih pasangan inventori"
            },
        },
        errorElement: 'span',
    ...
    $('.btn-submit-alias').click(function(e) {
        ...
        if ($('#update-alias-form').valid()) {
            ...
            formData.append('barang_master_id', barangMasterId);

            $.ajax({
                url: "<?= base_url("alias-barang/update-sales"); ?>",
                data: formData,
    ...
    function edit(id, barang_sales_name) {
        $('#id').val(id);
        $('#barang_master_id').val(null).change();
        $('#barang_sales_name').val(barang_sales_name).attr('readonly', true);
