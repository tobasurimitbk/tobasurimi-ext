                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            if (id) {
                                // UPDATE
                                $.ajax({
                                    url: "<?= base_url("retur-po-lokal-bp/update"); ?>",
                                    data: data,
                                ...
                                // INSERT
                                $.ajax({
                                    url: "<?= base_url("retur-po-lokal-bp/save"); ?>",
                                    data: data,
                                    beforeSend: function(xhr) {
                                        xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                        setLoading();
    ...
    function drawTable(listBarang) {
        ...
        if (listBarang.length == 0) {
            ...
            if (sumberBarang === "-") {
                Swal.fire({
                    icon: 'error',
                    title: "LPB yang anda pilih untuk retur belum dibuatkan dokumen pemasukan barang oleh Bea Cukai, silahkan dicek kembali",
                    confirmButtonColor: '#4e73df',
                    confirmButtonText: 'Ok'
                });
