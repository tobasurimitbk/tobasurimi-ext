...
<section class="section">
    <div class="section-header">
        <h1 class="title-name">Retur Purchase Lokal Import Bahan Penolong</h1>
        <div class="col-button-tambah-spp">
            <a class="btn btn-hide-form btn-discard float-right" href="<?= base_url("retur-po-import-bp"); ?>">
                Kembali
            </a>
            <?php if (!empty($dataPengembalianBarang)) : ?>
                <?php if (can('Retur Pembelian', 'Retur Import BP', 'p')) : ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("retur-po-import-bp/print/"); ?><?= encrypt($dataPengembalianBarang['id']); ?>')">
                        Print
                    </button>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <div class="card">
        <div class="card-body">
            <form class="create-form form-add-spp" role="form" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="type_return" id="type_return" value="IMPORT PENOLONG">
                <div class="row">
                    <div class="col-md-4">
...
<script>
            ...
            if (listBarang.length === 0) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let url = id == '' ? '<?= base_url("retur-po-import-bp/save"); ?>' : '<?= base_url("retur-po-import-bp/update"); ?>';
                            ...
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            window.location.href = "<?= base_url("retur-po-import-bp") ?>";
                                        }
                                    });
                                } else {
                                    Swal.fire({
    ...
    function sumTotalRetur() {
        ...
        $('.jumlah_retur').each(function() {
            ...
            var jumlahDiterimaText = $row.find('td:nth-child(6)').text().trim();
            var hargaText = $row.find('td:nth-child(8)').text().trim();
    ...
    function drawTable(listBarang) {
        ...
        if (listBarang.length == 0) {
            ...
            var newRow = $('<tr>');
            newRow.append($('<td style="text-align:right;" colspan="9"><b>GRAND TOTAL</b></td>'));
    ...
    function getPenerimaanBarangDetail() {
        ...
        $.ajax({
            url: `<?= base_url('retur-po-import-bp/retur-detail'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function getPenerimaanBarang() {
        $.ajax({
            url: `<?= base_url('retur-po-import-bp/dropdown-penerimaan-barang'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function posting(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bp/posting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
                    ...
                    success: function(response) {
                        ...
                        if (response.status) {
                            Swal.fire({
                                ...
                                location.href = "<?= base_url('retur-po-import-bp') ?>"
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
    ...
    function unposting(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bp/unposting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
    ...
    function remove(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bp/delete"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
