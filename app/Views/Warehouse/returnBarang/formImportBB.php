...
<section class="section">
    <div class="section-header">
        <h1 class="title-name">Retur Purchase Lokal Import Bahan Baku</h1>
        <div class="col-button-tambah-spp">
            <a class="btn btn-hide-form btn-discard float-right" href="<?= base_url("retur-po-import-bb"); ?>">
                Kembali
            </a>
            <?php if (!empty($dataPengembalianBarang)) : ?>
                <?php if (can('Retur Pembelian', 'Retur Import BB', 'p')) : ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("retur-po-import-bb/print/"); ?><?= encrypt($dataPengembalianBarang['id']); ?>')">
                        Print
                    </button>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <div class="card">
        <div class="card-body">
            <form class="create-form form-add-spp" role="form" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="type_return" id="type_return" value="IMPORT BAKU">
                <div class="row">
                    <div class="col-md-4">
...
<script>
            ...
            if (listBarang.length === 0) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let url = id == '' ? '<?= base_url("retur-po-import-bb/save"); ?>' : '<?= base_url("retur-po-import-bb/update"); ?>';
                            ...
                            success: function(response) {
                                if (response.status) {
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            window.location.href = "<?= base_url("retur-po-import-bb") ?>";
                                        }
                                    });
                                } else {
                                    Swal.fire({
    ...
    function getPenerimaanBarang() {
        $.ajax({
            url: `<?= base_url('retur-po-import-bb/dropdown-penerimaan-barang'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function posting(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bb/posting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
    ...
    function unposting(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bb/unposting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
    ...
    function remove(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("retur-po-import-bb/delete"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
