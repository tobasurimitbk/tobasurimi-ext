...
<section class="section">
    <div class="section-header">
        <h1 class="title-name">Return Barang Bahan Baku</h1>
        <div class="col-button-tambah-spp">
            ...
            <?php if (!empty($dataPenerimaanBarang)) : ?>
                <?php if (can('Warehouse', 'P. Barang Lokal BB', 'p')) : ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("penerimaan-barang-import-bb/return-barang/print/"); ?><?= encrypt($dataPenerimaanBarang['id']); ?>')">
                        Print
                    </button>
                <?php endif; ?>
            <?php endif; ?>
...
<script>
    ...
    $('.multiple_po_id').select2({
        ...
        $.ajax({
            url: `<?= base_url("penerimaan-barang-import-bb/return-barang/list-barang"); ?>`,
            method: "GET",
            data: {
    ...
    $('.btn-submit-parent').click(function(e) {
        ...
        if ($('.create-form').valid()) {
            ...
            if (!isValid) {
                Swal.fire({
                    icon: 'error',
                    title: "Jumlah Barang di return harus diisi atau tidak boleh 0",
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (pengembalian_barang_id) {
                            ...
                            $.ajax({
                                url: "<?= base_url("penerimaan-barang-import-bb/return-barang/update"); ?>",
                                data: formData,
                                method: "POST",
                                dataType: "json",
                                processData: false,
                            ...
                            $.ajax({
                                url: "<?= base_url("penerimaan-barang-import-bb/return-barang/insert"); ?>",
                                data: formData,
                                method: "POST",
                                dataType: "json",
                                processData: false,
    ...
    function changeStatus() {
        ...
        if (value) {
            ...
            $.ajax({
                url: `<?= base_url("penerimaan-barang-import-bb/return-barang/generate-po-no"); ?>`,
                method: "GET",
                data: {
        ...
        $('.posting-lpb').click(function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const csrf = $(`[name="${csrfToken}"]`);
                    $.ajax({
                        url: "<?= base_url("penerimaan-barang-import-bb/return-barang/posting"); ?>",
                        data: {
                            id: $('.id').val()
                        },
                        beforeSend: function(xhr) {
        ...
        $('.delete-parent').click(function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const csrf = $(`[name="${csrfToken}"]`);
                    $.ajax({
                        url: "<?= base_url("penerimaan-barang-import-bb/return-barang/delete"); ?>",
                        data: {
                            id: $('.id').val()
                        },
                        beforeSend: function(xhr) {
