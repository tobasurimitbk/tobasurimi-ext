        ...
        <div class="col-button-tambah-spp">
            ...
            <?php if (!empty($dataPenerimaanBarang)) : ?>
                <?php if (can('Warehouse', 'P. Barang Import BP', 'p')) : ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("penerimaan-barang-import-bp/print/"); ?><?= encrypt($dataPenerimaanBarang['id']); ?>')">
                        Print
                    </button>
                <?php endif; ?>
            <?php endif; ?>
            <?php if (!empty($dataPenerimaanBarang)) : ?>
                <?php if ($dataPenerimaanBarang['status_post'] === "WAITING") : ?>
                    ...
                    <?php endif; ?>
                    <?php if (can('Warehouse', 'P. Barang Import BP', 'u')) : ?>
                    ...
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <div class="input-group input-group-password">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <input readonly autocomplete="one-time-code" <?= !empty($dataPenerimaanBarang) ? ($dataPenerimaanBarang['status_post'] === "FINISH" ? 'disabled=true' : '') : ''; ?> value="<?= !empty($dataPenerimaanBarang) ? $dataPenerimaanBarang['no_penerimaan_barang'] : "LPB-IBP//" . date('m') . "/1/" . date('Y'); ?>" type="text" class="form-control no_penerimaan_barang" id="no_penerimaan_barang" name="no_penerimaan_barang" placeholder="No. Penerimaan">
...
<script>
    ...
    $('.supplier_id').select2({
        ...
        // GET DIVISI
        $.ajax({
            url: `<?= base_url('penerimaan-barang-import-bp/get-divisi'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    $('.divisi_id').select2({
        ...
        // GET WAREHOUSES
        $.ajax({
            url: `<?= base_url('penerimaan-barang-import-bp/warehouse'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (id) {
                            ...
                            $.ajax({
                                url: "<?= base_url("penerimaan-barang-import-bp/update"); ?>",
                                data: formData,
                                method: "POST",
                                dataType: "json",
                                processData: false,
                            ...
                            $.ajax({
                                url: "<?= base_url("penerimaan-barang-import-bp/insert"); ?>",
                                data: formData,
                                method: "POST",
                                dataType: "json",
                                processData: false,
        ...
        if (jml_diterima_lpb == 0) {
            for (var i = 0; i < listData.result.length; i++) {
                if (Number(listData.result[i].am_purchase_order_details_id) == Number($('.am_purchase_order_details_id').val()) && Number(listData.result[i].am_purchase_order_id) == Number($('.am_purchase_order_id').val())) {
                    ...
                    var jml_diterima_total_now = Math.floor(Number(item.jml_diterima_total - jml_diterima_lpb_last) * 1000) / 1000;
            ...
            for (var i = 0; i < listData.result.length; i++) {
                if (Number(listData.result[i].am_purchase_order_details_id) == Number($('.am_purchase_order_details_id').val()) && Number(listData.result[i].am_purchase_order_id) == Number($('.am_purchase_order_id').val())) {
                    item = listData.result[i];
                    var jml_diterima_total_now = Math.floor((Number(item.jml_diterima_total) + Number(jml_diterima_lpb) - jml_diterima_lpb_last) * 1000) / 1000;
    ...
    function changeStatus() {
        ...
        if (value) {
            ...
            $.ajax({
                url: `<?= base_url("penerimaan-barang-import-bp/generate-po-no"); ?>`,
                method: "GET",
                data: {
