        ...
        <div class="col-button-tambah-spp">
            <?php if ($akunCeisa != null) : ?>
                <?php if ($akunCeisa['status_integrasi']) : ?>
                    <a href="<?= base_url('bea-cukai-bc-30/bc-30-outstanding') ?>" class="btn btn-save float-right" type="button">
            ...
            <div class="row justify-content-start row-col-spp">
                <div class="col-md-3 mb-3">
                    ...
                    <div class="input-group input-group-password">
                        <input autocomplete="one-time-code" class="form-control input-picker dateStart" id="dateStart" name="dateStart" placeholder="Mulai Tanggal" value="<?= date('d/m/Y') ?>">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title title-secondary">Tambah Dokumen Pengeluaran BC 3.0</h5>
            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-floating mb-3 mt-1" style="height: 50px;">
                                <select class="form-select jenis_pengeluaran" id="jenis_pengeluaran" name="jenis_pengeluaran">
                                    <option value=""></option>
                        ...
                        <div class="col-sm-12">
                            <div class="form-floating mb-3 mt-1" style="height: 50px;">
                                <select class="form-select reference_penerima_id" id="reference_penerima_id" name="reference_penerima_id">
                                    <option value=""></option>
                                    <?php foreach ($customer as $c): ?>
                                        <option value="<?= $c['id'] ?>"><?= $c['name'] ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <label style="z-index: 1;">Pilih Customer (Optional)</label>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        columns: [{
...

            {
                ...
                render: function(data, type, row) {
                    let htmlRes = `
                        <a href="<?= base_url("bea-cukai-bc-30/id"); ?>/${row.id}" data-toggle="tooltip" title="Edit" class="btn btn-primary">
        ...
        language: {
            emptyTable: "Tidak ada riwayat dokumen BC 3.0", // Change this line
    ...
    $('#btnSubmitTambah').click(function(e) {
        if ($('.create-form').valid()) {
            ...
            var referencePenerimaId = $('#reference_penerima_id option:selected').val();
            var isValid = true;

            if (jenisPengeluaran == "ORDER FORM EKSPOR") {
                if (referencePenerimaId == "") {
                    isValid = false;
                }
            }
            ...
            if (!isValid) {
                ...
                formData.append("reference_penerima_id", referencePenerimaId);
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status) {
                            var id = res.id;
                            window.location.href = "<?= base_url('bea-cukai-bc-30/id/'); ?>" + id;
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: res.message,
    ...
    $('#btnTambahModal').click(function(e) {
        ...
        $('#jenis_pengeluaran').val(null).change();
    ...
    $('.dateStart, .dateEnd,.statusPosting,.tipeSalesOrder').change(function() {
        table.ajax.reload();
    });
