    ...
    <div class="card">
        ...
        <div class="card-body">
                ...
                <div class="row">
                    <div class="col-sm-3">
                        <div class="form-floating mb-3 mt-1" style="height: 50px;">
                            <select disabled class="form-select jenis_pengeluaran" id="jenis_pengeluaran" name="jenis_pengeluaran">
                                <option value=""></option>
                                <option <?= $bc30['jenis_pengeluaran'] == "ORDER FORM EKSPOR" ? "selected" : "" ?> value="ORDER FORM EKSPOR">ORDER FORM EKSPOR</option>
                                <option <?= $bc30['jenis_pengeluaran'] == "LAINNYA" ? "selected" : "" ?> value="LAINNYA">LAINNYA</option>
                            </select>
                    ...
                    <div class="col-sm-3">
                        <div class="form-floating mb-3">
                            <input autocomplete="one-time-code" type="text" class="form-control tanggal" id="tanggal" name="tanggal" placeholder="Tanggal" value="<?= date('d/m/Y', strtotime($bc30['tanggal'])) ?>">
                    ...
                    <div class="col-sm-3">
                        <div class="input-group">
                            <div class="form-floating mb-3">
                                <input autocomplete="one-time-code" value="<?= $bc30['no_daftar'] ?>" type="number" class="form-control no_daftar" id="no_daftar" name="no_daftar" placeholder="No Daftar (Opsional)">
                                <label for="floatingInput">Nomor Daftar (Opsional)</label>
                            </div>
                            <?php if (!empty($bc30)): ?>
                                <?php if ($bc30['status_dokumen'] == "Sudah Kirim"): ?>
                                    <div class="input-group-append" style="height:50px;">
                    ...
                    <div class="col-sm-3">
                        <div class="form-floating mb-3 mt-1" style="height: 50px;">
                            <select class="form-select reference_penerima_id" disabled id="reference_penerima_id" name="reference_penerima_id">
                                <option value=""></option>
                                <?php foreach ($dataCustomer as $d): ?>
                                    <option selected value="<?= $d['id'] ?>"><?= $d['name'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <label style="z-index: 1;">Pilih Penerima</label>
                        </div>
                    </div>
                    <div class="col-sm-4">
            ...
            <div class="col-subtitle-modal">
                <div class="row mt-3">
                    <div class="col-md-6">
                        <label class="form-label font-weight-bold modal-sub-title">List Barang Sales Ekspor (Marketing)</label>
                    </div>
                    <div class="col-md-6">
                    </div>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered nowrap table-hover-tobasurimi dataTable" id="dataTableEkspor" width="100%" border="1" cellspacing="0">
                    <thead class="thead-dark">
                        <tr>
                            <th style="text-align: center; width:5px;">No</th>
                            <th>Barang Sales Ekspor</th>
                            <th>Qty</th>
                            <th>Satuan</th>
                            <th>Valas</th>
                            <th>Nilai Barang</th>
            ...
            <div class="col-subtitle-modal">
                <div class="row mt-3">
                    <div class="col-md-6">
                        <label class="form-label font-weight-bold modal-sub-title">List Barang Keluar (Inventori)</label>
                    </div>
                    <div class="col-md-6">
...
<script>
    ...
    var listStock = [];
    var listBarangSalesEkspor = [];

    listStock = <?= json_encode($dataBarang) ?>;
    listBarangSalesEkspor = <?= json_encode($dataBarangSalesEkspor) ?>;
    drawTable(listStock);
    drawTableBarangSalesEkspor(listBarangSalesEkspor);
    ...
    var table = $('.table-inventori').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url('bea-cukai-bc-30/all-stock-list'); ?>",
            type: "GET",
            data: function(data) {
    ...
    $('#btnSubmitUpdateNoAju').click(function(e) {
        ...
        if ($('#form-update-noaju').valid()) {
            ...
            data.append("id", "<?= encrypt($bc30['id']) ?>");
            data.append("no_pengajuan", no_pengajuan);
            $.ajax({
                url: "<?= base_url("bea-cukai-bc-30/update-no-aju"); ?>",
                data: data,
    ...
    $('.btn-submit-parent').click(function() {
        ...
        if (jenis_pengeluaran == '') {
            ...
            if ($('.create-form').valid()) {
                ...
                if (jenis_pengeluaran == "ORDER FORM EKSPOR") {
                ...
                if (!isFalid) {
                    ...
                    var id = $('.id').val();
                    var url = "<?= base_url("bea-cukai-bc-30/update"); ?>";
    ...
    function detail(id) {
        ...
        if (item == '') {
            ...
            $.ajax({
                url: "<?= base_url("bea-cukai-bc-30/list-satuan-konversi"); ?>",
                data: {
                    id: id
                },
                method: "GET",
    ...
    function drawTableBarangSalesEkspor(listBarangSalesEkspor) {
        const table = $('#dataTableEkspor');
        table.find('tbody').empty();
        table.find('tfoot').empty();
        var no = 1;
        if (listBarangSalesEkspor.length == 0) {
            var newRow = $('<tr>');
            newRow.append($('<td colspan="6">').text("Tidak Ada Data"));
            table.find('tfoot').append(newRow);
        } else {
            var no = 1;
            var totalNilaiBarang = 0;
            $.each(listBarangSalesEkspor, function(i, v) {
                var newRow = $('<tr style="color:whitesmoke;">');
                newRow.append($('<td>').text(no++));
                newRow.append($('<td>').text(v.barang_name));
                newRow.append($('<td>').text(greatFormatRupiah(parseFloat(v.qty).toFixed(2))));
    ...
    function dropdownWarehouse() {
        $.ajax({
            url: `<?= base_url('bea-cukai-bc-30/warehouse'); ?>`,
            method: "GET",
            data: {
    ...
    function dropdownBarangSalesEkspor() {
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status) {
                    console.log(res);
                    listBarangSalesEkspor = res.data;
                    drawTableBarangSalesEkspor(listBarangSalesEkspor);
                } else {
                    Swal.fire({
                        icon: 'error',
