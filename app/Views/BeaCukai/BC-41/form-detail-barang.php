        ...
        <div class="card-body">
            ...
            <div class="detail-barang-form-view">
                <div class="mt-3">
                    ...
                    <a class="btn btn-danger float-right" href="<?= base_url('bea-cukai-bc-41/id/barang/' . encrypt($bc41['id'])) ?>" style="margin-right: 8px;">
                        Batal
                    </a>
                </div>
                ...
                <form id="form-barang-detail">
                    <div class="row">
                        ...
                        <div class="col-sm-4 mt-1">
                            ...
                            <div class="mt-1">
                                <div class="form-floating mb-3">
                                    <input id="volume" name="volume" type="text" class="form-control volume" value="<?= $barang['bcDetail']->volume ?>" placeholder="">
                        ...
                        <div class="col-sm-4 mt-1">
                            ...
                            <div class="mt-1">
                                <div class="form-floating mb-3">
                                    <input id="nilaiJasa" name="nilaiJasa" onchange="this.value = formatRupiah(this.value)" type="text" class="form-control nilaiJasa" value="<?= formatRupiah($barang['bcDetail']->nilaiJasa) ?>" placeholder="">
                                    <label>Nilai Penggantian / Jasa</label>
                                </div>
                            </div>
                    ...
                    <div class="col-sm mt-1">
                        ...
                        <table class="table table-bordered nowrap table-hover-tobasurimi dataTable table-list-informasi-pungutan" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <?php else : ?>
                                    <?php $no = 1; ?>
                                    <?php foreach ($bahanBakuLokal as $i => $b) : ?>
                                        <tr style="color: white; text-align:center;">
                                            <td><?= $no++; ?></td>
                                            <td><?= $b['posTarif']; ?></td>
            ...
            if (dataSeriBarang.length == 0) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "<?= base_url("bea-cukai-bc-41/id/barang/bahan-baku-create"); ?>",
                            data: formData,
                            beforeSend: function(xhr) {
                                xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                setLoading();
        ...
        if ($('#form-barang-detail').valid()) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "<?= base_url("bea-cukai-bc-41/id/barang"); ?>",
                        data: formData,
                        beforeSend: function(xhr) {
                            xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                            $('#btn-loading').show();
    ...
    function removeBahanBaku(indexDelete) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("bea-cukai-bc-41/id/barang/bahan-baku-delete"); ?>",
                    data: {
                        id: "<?= encrypt($bc41['id']) ?>",
