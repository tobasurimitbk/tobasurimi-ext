                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            ...
                            <div class="input-group input-group-password">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <input disabled readonly value="<?= count($payload->entitas) == 0 ? "" : date('d/m/Y', strtotime($payload->entitas[0]->tanggalIjinEntitas)) ?>" autocomplete="one-time-code" name="entitas_tanggal_skep_tpb" type="text" placeholder="" class="form-control entitas_tanggal_skep_tpb" id="entitas_tanggal_skep_tpb">
                        ...
                        <div class="col-sm-4">
                            <label class="form-label font-weight-bold lable-title mt-4 mb-2">
                                TPB Tujuan Barang / Pembeli Barang Kena Pajak / Penerima Jasa Kena Pajak / Penerima Barang
                            </label>
                            <div class="mt-1">
                                <div class="form-floating mb-3">
                            ...
                            <div class="mt-1">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <select class="form-select entitas_nomor_ijin_tpb_penerima_barang" id="entitas_nomor_ijin_tpb_penerima_barang" name="entitas_nomor_ijin_tpb_penerima_barang" aria-label="Floating label select example">
                                        <option selected value="<?= count($payload->entitas) != 0 ? $payload->entitas[2]->nomorIjinEntitas : '' ?>"><?= count($payload->entitas) != 0 ? $payload->entitas[2]->nomorIjinEntitas : '' ?></option>
                                    </select>
                                    <label style="z-index: 1;">Nomor Izin TPB</label>
                                </div>
                            </div>
                            <div class="input-group input-group-password">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <input disabled readonly value="<?= count($payload->entitas) == 0 ? "" : date('d/m/Y', strtotime($payload->entitas[2]->tanggalIjinEntitas)) ?>" autocomplete="one-time-code" name="entitas_tanggal_skep_tpb_penerima_barang" type="text" placeholder="" class="form-control entitas_tanggal_skep_tpb_penerima_barang" id="entitas_tanggal_skep_tpb_penerima_barang">
                                    <label>Tanggal Skep TPB</label>
                                </div>
                                <div class="input-group-prepend group-prepend-password align-items-center">
                                    <i style="cursor: pointer; z-index: 99; margin-bottom: 20px; margin-left: -30px; border: 0px" class="fa fa-calendar icon-form icon-po-date"></i>
...
<script>
    ...
    $('#entitas_nomor_ijin_tpb_penerima_barang').select2({
        placeholder: "Pilih No Ijin TPB",
        theme: "bootstrap-5",
    }).change(function() {
        var selected = $(this).find('option:selected');
        $('#entitas_tanggal_skep_tpb_penerima_barang').val(selected.data('tanggal_skep_tpb'));
        $('#entitas_alamat_pemilik_barang').val(selected.data('alamat_pemilik_barang'));
    });
    ...
    $("#entitas_tanggal_skep_tpb_penerima_barang").datepicker({
        todayHighlight: true,
        format: "dd/mm/yyyy",
        orientation: "bottom auto",
        autoclose: true
        ...
        if ($('#form-entitas').valid()) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    formData.append("entitas_tanggal_skep_tpb_penerima_barang", $('#entitas_tanggal_skep_tpb_penerima_barang').val());
                    $.ajax({
                        url: "<?= base_url("bea-cukai-bc-27/id/entitas"); ?>",
                        data: formData,
