                    ...
                    <div class="col-sm-3">
                        <div class="form-floating mb-3 mt-1" style="height: 50px;">
                            <select class="form-select jenis_pengeluaran" id="jenis_pengeluaran" name="jenis_pengeluaran" aria-label="Floating label select example">
                                <option value=""></option>
                                <option <?= $bc25['jenis_pengeluaran'] == "ORDER FORM LOKAL" ? "selected" : "" ?> value="ORDER FORM LOKAL">ORDER FORM LOKAL</option>
                                <option <?= $bc25['jenis_pengeluaran'] == "LAINNYA" ? "selected" : "" ?> value="LAINNYA">LAINNYA</option>
                            </select>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-3 form-fifo">
                                <div class="form-floating" style="height: 50px;">
                                    <input placeholder="Qty Mutasi" oninput="this.value = greatFormatRupiah(this.value)" class="form-control qty_keluar_fifo" id="qty_keluar_fifo" name="qty_keluar_fifo" />
...
<script>
    ...
    var table = $('.table-inventori').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url('bea-cukai-bc-25/all-stock-list'); ?>",
            type: "GET",
            data: function(data) {
        ...
        if (jenis_pengeluaran == '') {
            ...
            if ($('.create-form').valid()) {
                ...
                if (!isFalid) {
                    ...
                    var id = $('.id').val();
                    var url = "<?= base_url("bea-cukai-bc-25/update"); ?>";
    ...
    function detail(id) {
        ...
        if (item == '') {
            ...
            $.ajax({
                url: "<?= base_url("bea-cukai-bc-25/list-satuan-konversi"); ?>",
                data: {
                    id: id
                },
                method: "GET",
    ...
    function dropdownWarehouse() {
        $.ajax({
            url: `<?= base_url('bea-cukai-bc-25/warehouse'); ?>`,
            method: "GET",
            data: {
