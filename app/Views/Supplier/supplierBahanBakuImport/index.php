    ...
    <div class="modal-dialog" style="min-width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><label class="title-name"></label>Data Supplier Bahan Baku Import</h5>
            </div>
...
<div class="section-header">
    <h1>Supplier Bahan Baku Import</h1>
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("supplier-bahan-baku-import/all"); ?>",
            dataSrc: "data",
            data: function(data) {
    ...
    $(document).ready(function() {
        ...
        $('#dataTable tbody').on('click', 'tr td:not(.actions):not(.dataTables_empty)', function() {
            ...
            $.ajax({
                url: "<?= base_url("supplier-bahan-baku-import/id"); ?>" + "/" + id,
                method: "GET",
                dataType: "json",
                success: function(res) {
                    if (res.status) {
        ...
        $(".delete-form").click(function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    setLoading()
                    $.ajax({
                        url: "<?= base_url("supplier-bahan-baku-import/delete"); ?>",
                        data: {
                            id: id
                        },
                ...
                if ($(".create-form").valid()) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: id ? "<?= base_url("supplier-bahan-baku-import/update"); ?>" : "<?= base_url("supplier-bahan-baku-import/save"); ?>",
                                data: data,
                                beforeSend: function(xhr) {
                                    xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                },
