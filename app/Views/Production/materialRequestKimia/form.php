        ...
        <div class="col-button-tambah-spp text-right">
            ...
            <?php if (isset($dataMaterialRequests)) { ?>
                <?php if ($dataMaterialRequests->is_posted != 1) { ?>
                    <?php if (can('Produksi', 'Material Request Kimia', 'a')) : ?>
                        <button class="btn btn-success mr-1" onclick="posting('<?= !empty($ids) ? $ids : ''; ?>', 1)">
                            Posting
                        </button>
                    <?php endif; ?>
                    <?php if (can('Produksi', 'Material Request Kimia', 'd')) : ?>
                ...
                <?php } ?>
                <?php if (can('Produksi', 'Material Request Kimia', 'p')) : ?>
                    <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("material-request-kimia/print/"); ?><?= encrypt($dataMaterialRequests->id) ?>')">
                        Print
                    </button>
                <?php endif; ?>
...
<script>
    ...
    const changeStatus = function() {
        ...
        if (value) {
            $.ajax({
                url: `<?= base_url("/material-request-kimia/generate-kode-request"); ?>`,
                method: "GET",
                dataType: "json",
