        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                newRow3.append($('<td style="text-align:center;"><b>' +
                    `<input autocomplete="one-time-code" data-id="" onchange="this.value = formatRupiah(this.value)" class="form-control potongan trigger-input" type="text" value="<?= !empty($detail) ? formatRupiah($detail['potongan'])  : '' ?>" name="potongan" oninput="limitInputBayar(this, ${limit_bayar})">` +
                    '</b></td>'));
