...
<section class="section">
    <div class="section-header">
        ...
        <div class="col-button-tambah-spp">
            <a class="btn btn-hide-form btn-discard float-right" href="<?= base_url("pembayaran-invoice-export"); ?>">
                Kembali
            </a>
            ...
            <?php if (!empty($detail)) : ?>
                <?php if ($detail['status_posting'] == "0") : ?>
                    ...
                    <?php endif; ?>
                    <?php if (can('Transaksi Internasional', 'Pembayaran Invoice', 'u')) : ?>
                            ...
                            <select <?= !empty($detail) ? ($detail['status_posting'] == 1 ? 'disabled' : '') : ""  ?> class="form-select no_dokumen" name="no_dokumen" id="no_dokumen">
                                ...
                                <?php if (!empty($dokumenList)): ?>
                                    <?php foreach ($dokumenList as $l): ?>
                                        <option <?= (!empty($detail)) ?  (($detail['invoice_id']) == $l['sales_order_export_id'] ? "selected" : "") : '' ?> value="<?= encrypt($l['sales_order_export_id']); ?>"><?= $l['no_invoice']; ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
