...
<section class="section">
    <div class="section-header">
        <h1>Pengeluaran Barang Internasional</h1>
        <?php if (can("Stuffing", "Pengeluaran Internasional", "c")) : ?>
            <a href="<?= base_url('pengeluaran-internasional/create') ?>" type="button" class="btn btn-show-form btn-add float-right">
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("pengeluaran-internasional/all"); ?>",
            dataSrc: "data",
            data: function(data) {
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    `
                        ...
                        //         <button data-toggle="tooltip" title="Print" class="btn btn-warning btn-print" onclick="print('<?= base_url("pengeluaran-internasional/print/"); ?>${id}')" style="box-shadow: none !important;">
    ...
    $('#dataTable tbody').on('click', 'tr td:not(.actions):not(.dataTables_empty)', function() {
        ...
        location.replace(`<?= base_url("pengeluaran-internasional/id"); ?>/${data.id}`);
    });
    ...
    const closed = function(id) {
        Swal.fire({
            icon: 'question',
            title: 'Close Stuffing Pengeluaran Internasional ?',
            confirmButtonColor: '#4e73df',
            cancelButtonColor: '#d33',
            showCancelButton: true,
            reverseButtons: true,
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("pengeluaran-internasional/close"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
