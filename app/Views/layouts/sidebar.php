                    ...
                    <li class="nav-item dropdown <?= ($exists ? "active" : "{{ ' active'|is_active('^index(.*)', page)|safe }}") ?>">
                        ...
                        <ul class="dropdown-menu">
                            <?php foreach ($parent_access->child as $child_access) {
                                if (can($parent_access->menuName, $child_access->name, "r")) {
                                    if ($currentUriSegment  === $child_access->url) { ?>
                                        <li{{ ' class="active"'|is_active('^index-0.html', page)|safe }}><a class="nav-link active-bar" href="<?= base_url($child_access->url); ?>"><?= $child_access->name; ?></a>
                    </li>
                <?php } else { ?>
