...
<section class="section">
    <div class="section-header">
        <h1>Jasa Vendor Barang Keluar - Kepiting Kukus</h1>
        <?php if (can("Jasa Vendor", "Barang Keluar", "c")) : ?>
            <a href="<?= base_url('jasa-vendor-out-kepiting-kukus/create') ?>" type="button" class="btn btn-show-form btn-add float-right">
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("jasa-vendor-out-kepiting-kukus/all"); ?>",
            dataSrc: "data",
            data: function(data) {
    ...
    $('#dataTable tbody').on('click', 'tr td:not(.actions):not(.dataTables_empty)', function() {
        ...
        location.replace(`<?= base_url("jasa-vendor-out-kepiting-kukus/id"); ?>/${data.id}`);
    });
    ...
    const unPosting = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("jasa-vendor-out-kepiting-kukus/unposting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
