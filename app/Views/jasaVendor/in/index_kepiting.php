...
<section class="section">
    <div class="section-header">
        ...
        <?php if (can("Jasa Vendor", "Barang Masuk", "c")) : ?>
            <a href="<?= base_url('jasa-vendor-in-kepiting/create') ?>" type="button" class="btn btn-show-form btn-add float-right">
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("jasa-vendor-in-kepiting/all"); ?>",
            dataSrc: "data",
            data: function(data) {
    ...
    $('#dataTable tbody').on('click', 'tr td:not(.actions):not(.dataTables_empty)', function() {
        ...
        location.replace(`<?= base_url("jasa-vendor-in-kepiting/id"); ?>/${data.id}`);
    });
    ...
    const postingBayar = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("jasa-vendor-in-kepiting/posting-bayar"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
    ...
    const unPosting = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("jasa-vendor-in-kepiting/unposting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
