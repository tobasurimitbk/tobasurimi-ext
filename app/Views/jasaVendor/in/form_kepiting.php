...
<section class="section">
    <div class="section-header">
        ...
        <div class="col-button-tambah-spp">
            <a class="btn btn-hide-form btn-discard float-right" href="<?= base_url("jasa-vendor-in-kepiting"); ?>">
                Kembali
            </a>
            <?php if (!empty($jasaVendorIn)) : ?>
                <?php if ($jasaVendorIn['status_posting'] == "0") : ?>
                    ...
                    <?php if (can('Jasa Vendor', 'Barang Masuk', 'p')) : ?>
                        <button class="btn btn-warning btn-print float-right" onclick="print('<?= base_url("jasa-vendor-in-kepiting/print/"); ?><?= encrypt($jasaVendorIn['id']); ?>')">
                            Print
                        </button>
                    <?php endif; ?>
                    ...
                    <div class="col-md-4">
                        <div class="form-floating mb-3" style="height: 50px;">
                            <div class="input-group input-group-password">
                                <div class="form-floating mb-3" style="height: 50px;">
                                    <input <?= !empty($jasaVendorIn['status_posting']) ? 'readonly' : '' ?> autocomplete="one-time-code" class="form-control input-picker tanggal" id="tanggal" name="tanggal" placeholder="Tanggal Dibuat" value="<?= date('d/m/Y', strtotime(!empty($jasaVendorIn) ? $jasaVendorIn['tanggal'] : $tanggal)); ?>">
...
<script>
    ...
    <?php if (!empty($jasaVendorIn)) : ?>
        ...
        $.ajax({
            url: `<?= base_url("jasa-vendor-in-kepiting/list-barang"); ?>`,
            method: "GET",
            data: {
        ...
        if (listBarang.length == 0) {
            ...
            if ($('.create-form').valid()) {
                ...
                if (!isValidBarang) {
                    ...
                    // dataDetail = array detail dari backend
                   ...
                   $.each(listBarangGroup, function(i, gb) {
                        ...
                        if (detailMatch) {
                            // Add the required fields to the group object
                            gb.jasa_vendor_out_id = detailMatch.jasa_vendor_out_id;
                            gb.jasa_vendor_out_detail_id = detailMatch.jasa_vendor_out_detail_id;
                            gb.stock_out_id = detailMatch.stock_out_id;
                            gb.bc_id = detailMatch.bc_id;
                            gb.no_aju = detailMatch.no_aju;
                            gb.stock_dokumen = detailMatch.stock_dokumen;
                        }
                    });
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            let oneRaw = $('#flexSwitchCheckPosting').is(':checked') ? 1 : 0;
                            data.append('one_raw', oneRaw);

                            data.append('listBarang', JSON.stringify(listBarangGroup));

                            if (id) {
                                // UPDATE
                                $.ajax({
                                    url: "<?= base_url("jasa-vendor-in-kepiting/update"); ?>",
                                    data: data,
                                    ...
                                    success: function(response) {
                                        if (response.status) {
                                            Swal.fire({
                                                ...
                                                if (result.isConfirmed) {
                                                    window.location.href = "<?= base_url("jasa-vendor-in-kepiting") ?>";
                                                }
                                            });
                                        } else {
                                            Swal.fire({
                                ...
                                // INSERT
                                $.ajax({
                                    url: "<?= base_url("jasa-vendor-in-kepiting/save"); ?>",
                                    data: data,
                                    beforeSend: function(xhr) {
                                        xhr.setRequestHeader('X-CSRF-Token', csrf.val());
                                        setLoading();
    ...
    function getListDivisi() {
        // GET LIST DIVISI
        $.ajax({
            url: `<?= base_url('jasa-vendor-in-kepiting/divisi'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function getListWarehouse() {
        // GET LIST DIVISI
        $.ajax({
            url: `<?= base_url('jasa-vendor-in-kepiting/warehouse'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function getListJasaVendorOut() {
        $.ajax({
            url: `<?= base_url('jasa-vendor-in-kepiting/list-jasa-vendor-out'); ?>`,
            method: "GET",
            beforeSend: function() {
                setLoading();
            },
    ...
    function changeStatus() {
        ...
        if (value) {
            ...
            $.ajax({
                url: `<?= base_url("jasa-vendor-in-kepiting/get-jasa-vendor-in-no"); ?>`,
                method: "GET",
                data: {
                    warehouse_id: $('#warehouse_id option:selected').val(),
                    tanggal: $('#tanggal').val()
    ...
    const posting = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("jasa-vendor-in-kepiting/posting"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
    ...
    const remove = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const csrf = $(`[name="${csrfToken}"]`);
                $.ajax({
                    url: "<?= base_url("jasa-vendor-in-kepiting/delete"); ?>",
                    data: {
                        id: id
                    },
                    beforeSend: function(xhr) {
