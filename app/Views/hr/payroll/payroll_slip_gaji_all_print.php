<!DOCTYPE html>
<html>
<title>Payroll <?= $yearMonth ?>, Dept. <?= $divisi['divisi'] ?> Bag.
    <?= $bagian == null ? 'All' : $bagian['nama_bagian'] ?>
</title>

<head>
    <style>
        body {
            height: 100%;
            font-family: 'Times New Roman', Times, serif;
            letter-spacing: 1px;
            font-size: 8;
        }

        @page {
            margin: 20px 10px 10px 10px;
        }

        .page-break {
            page-break-after: always;
        }

        .karyawan-row {
            border-bottom: 1px dashed #000;
            margin-bottom: 20px;
            margin-top: 20px;
        }

        .slip-container {
            width: 100%;
            border-collapse: collapse;
        }

        .slip {
            width: 100%;
            /* padding: 8px 10px; */
            border-right: 1px dashed #000;
        }

        .slip:last-child {
            border-right: none;
        }

        h4 {
            margin-left: 3px;
            margin-bottom: 3px;
            font-size: 12px;
            text-align: left;
            font-weight: normal;
        }

        table {
            width: 100%;
            border-collapse: collapse;
        }

        td {
            vertical-align: top;
            padding: 2px 3px;
        }

        hr {
            border: none;
            border-top: 1px solid #000;
            margin: 5px 0;
        }

        .signature {
            text-align: center;
            padding-top: 1px;
        }

        .copy-label {
            text-align: center;
            font-size: 7x;
            font-style: italic;
            margin-bottom: 5px;
        }
    </style>
</head>

<body>
    <table width="100%" cellpadding="5" cellspacing="0">

        <?php foreach (array_chunk($payrollData['data'], 2) as $chunk): ?>
            <tr>

                <?php foreach ($chunk as $p): ?>
                    <td width="50%" valign="top">

                        <div style=" width:100%;">

                            <h4 style="margin:5px 0;">TANDA TERIMA UPAH</h4>

                            <table style="line-height: 9px;" border="0">
                                <tr>
                                    <td colspan="3">Tenaga Kerja Harian Tetap</td>
                                </tr>
                                <tr>
                                    <td width="120px">Kode Karyawan</td>
                                    <td width="10px">:</td>
                                    <td><?= isset($p['employee']['nip']) ? $p['employee']['nip'] : '' ?></td>
                                </tr>
                                <tr>
                                    <td>Tahun / Bulan</td>
                                    <td>:</td>
                                    <td>
                                        Tahun : <?= explode('-', $p['payroll']['year_month'])[0] ?>
                                        Bulan : <?= explode('-', $p['payroll']['year_month'])[1] ?>
                                        Periode : 1
                                    </td>
                                </tr>
                                <tr>
                                    <td>No. Induk</td>
                                    <td>:</td>
                                    <td>
                                        <?= isset($p['employee']['nip']) ? $p['employee']['nip'] : '' ?>
                                        Bagian :
                                        <?= isset($p['employee']['nama_bagian']) ? $p['employee']['nama_bagian'] : '' ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Nama</td>
                                    <td>:</td>
                                    <td><?= isset($p['employee']['name']) ? $p['employee']['name'] : '' ?></td>
                                </tr>
                            </table>

                            <hr style="border:0.5px dashed #000; margin:5px 0;">

                            <table style="line-height: 9px;" border="0">
                                <tr>
                                    <td width="150px">Hari Kerja</td>
                                    <td width="5px">:</td>
                                    <td><?= $p['payroll']['hadir_final'] ?> Hari</td>
                                </tr>
                                <tr>
                                    <td><?= $tunjanganGajiPokok == null ? "" : ucfirst(strtolower($tunjanganGajiPokok['name'])) ?>
                                    </td>
                                    <td>:</td>
                                    <td>
                                        Rp <?= number_format($p['payroll']['nominal_gaji_harian'], 2, ',', '.') ?>/Hari
                                    </td>
                                </tr>
                                <tr>
                                    <td><?= $tunjanganCadangan == null ? "" : ucfirst(strtolower($tunjanganCadangan['name'])) ?>
                                    </td>
                                    <td>:</td>
                                    <td>
                                        Rp <?= number_format($p['payroll']['nominal_cadangan'], 2, ',', '.') ?>/Hari
                                    </td>
                                </tr>
                                <tr>
                                    <td>Total Gaji</td>
                                    <td>:</td>
                                    <td>
                                        Rp
                                        <?= number_format($p['payroll']['total_gaji_harian_plus_cadangan'], 2, ',', '.') ?>/Hari
                                        :
                                        Rp <?= number_format($p['payroll']['nominal_uang_gaji'], 2, ',', '.') ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Tunjangan Tidak Tetap</td>
                                    <td>:</td>
                                    <td>
                                        Rp <?= number_format($p['tunjanganTidakTetap'], 2, ',', '.') ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Lembur I</td>
                                    <td>:</td>
                                    <td><?= $p['totalLemburJamPertama'] ?> Jam</td>
                                </tr>
                                <tr>
                                    <td>Lembur II</td>
                                    <td>:</td>
                                    <td><?= $p['totalLemburJamKedua'] ?> Jam</td>
                                </tr>
                                <tr>
                                    <td>Total Uang Lembur I & II</td>
                                    <td>:</td>
                                    <td>
                                        Rp <?= number_format($p['payroll']['nominal_uang_lembur'], 2, ',', '.') ?>
                                    </td>
                                </tr>
                            </table>

                            <hr style="border:0.5px dashed #000; margin:5px 0;">

                            <table width="100%" style="line-height: 9px;">
                                <tr>
                                    <td width="150px">Total Gaji & Lembur</td>
                                    <td width="5px">:</td>
                                    <td>
                                        Rp <?= number_format(
                                            $p['payroll']['nominal_uang_gaji']
                                            + $p['payroll']['nominal_uang_lembur']
                                            + $p['uangMakan'],
                                            2,
                                            ',',
                                            '.'
                                        ) ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Potongan Pinjaman</td>
                                    <td>:</td>
                                    <td>
                                        Rp <?= number_format($p['totalPinjamanDiambil'], 2, ',', '.') ?>
                                    </td>
                                </tr>

                                <?php foreach ($p['perhitunganGaji'] as $g): ?>
                                    <tr>
                                        <td><?= ucfirst(strtolower($g['name'])) ?></td>
                                        <td>:</td>
                                        <td>
                                            Rp <?= number_format($g['nominal'], 2, ',', '.') ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </table>

                            <hr style="border:0.5px dashed #000; margin:5px 0;">

                            <table width="100%">
                                <tr>
                                    <td width="150px">Sisa Diterima</td>
                                    <td width="5px">:</td>
                                    <td>
                                        Rp <?= number_format($p['payroll']['nominal_gaji_diterima'], 2, ',', '.') ?>
                                    </td>
                                </tr>
                            </table>

                            <hr style="border:0.5px dashed #000; margin:5px 0;">

                            <table width="100%" style="margin-top:10px;">
                                <tr>
                                    <td align="center">
                                        JURU BAYAR<br><br><br>
                                        (__________________)
                                    </td>
                                    <td align="center">
                                        PENERIMA<br><br><br>
                                        (__________________)
                                    </td>
                                </tr>
                            </table>

                        </div>

                    </td>
                <?php endforeach; ?>

                <?php if (count($chunk) == 1): ?>
                    <td width="50%" style="border:1px solid #000;"></td>
                <?php endif; ?>

            </tr>
        <?php endforeach; ?>

    </table>
</body>

</html>