...
<section class="section">
    <div class="section-header">
        <h1>PO Import Bahan Penolong</h1>
        <?php if (can('Pembelian', 'PO Import BP', 'c')): ?>
            <a class="btn btn-show-form btn-add float-right" href="<?= base_url("po-import-bahan-penolong/create"); ?>">
...
<script>
    ...
    const table = $('.dataTable').DataTable({
        ...
        pageLength: 25,
        ajax: {
            url: "<?= base_url("po-import-bahan-penolong/all"); ?>",
            dataSrc: "data",
            data: function(data) {
                ...
                render: function(data, type, row) {
                    ...
                    if (status !== "1") {
                        ...
                        <div class="mt-0">
                            <?php if (can('Pembelian', 'PO Import BP', 'p')) : ?>
                                <button data-toggle="tooltip" title="Print" class="btn btn-warning btn-print" onclick="print('<?= base_url("po-import-bahan-penolong/print/"); ?>${id}')" style="box-shadow: none !important;">
                    ...
                    `
                        ...
                        if (status_penerimaan !== "CLOSED") {
                            ...
                            return `
                            <div class="mt-0">
                                <?php if (can('Pembelian', 'PO Import BP', 'ua')) : ?>
                                    ${un_posting_row}
                                <?php endif; ?>
    ...
    function displayHistory(id) {
        console.log(id);
        $.ajax({
            url: "<?= base_url("po-import-bahan-penolong/histori-lpb"); ?>",
            data: {
                id: id
            },
            beforeSend: function(xhr) {
