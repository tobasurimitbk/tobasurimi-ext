...
<section class="section">
    <div class="section-header">
        <h1 class="title-name"><?= (!empty($dataPOImport)) ? "Update PO Import Bahan Penolong" : "Tambah PO Import Bahan Penolong" ?></h1>
...
<script>
    ...
    function changeStatus() {
        ...
        if (value) {
            $.ajax({
                url: `<?= base_url("/po-import-bahan-penolong/generate-no-po"); ?>`,
                method: "GET",
                dataType: "json",
                data: {
                    po_date: $('#po_date').val()
    ...
    function drawTabel(listBarang) {
        ...
        $.each(listBarang, function(i, v) {
            ...
            newRow.append($('<td>').text(greatFormatRupiah(v.harga_satuan.toFixed(2))));
    ...
    $(document).keydown(function(e) {
        if (e.keyCode === 116) {
            e.preventDefault();
            $.ajax({
                url: `<?= base_url("/po-import-bahan-penolong/dropdown/get-barang-bahan-penolong"); ?>`,
                method: "GET",
                dataType: "json",
                success: function(res) {
